...
get_folder_info <-
  function(workAddress,
    ...
    # 排序
    fileNames <- str_sort(fileNames,locale = "zh_CN.UTF-8")
    fileNamesWithSuffix <- str_sort(fileNamesWithSuffix,locale = "zh_CN.UTF-8")
    filePath_source <- str_sort(filePath_source,locale = "zh_CN.UTF-8")
    filePath_target <- str_sort(filePath_target,locale = "zh_CN.UTF-8")
