...
"汇总各银行之总资产。"
def together_B_A_all(bank:BankCommercial, bankList:TypeState{1}):
    bank.A_all[bankList] = bank.A_BI_all[bankList] + bank.A_exBI[bankList]
    pass


"加总各银行之银行间总资产，通过银行间资产邻接矩阵。"
def sum_B_A_BI(bank:BankCommercial, interbank:BankInterbank, bankList:TypeState{1}, interbankList:TypeState{2}):
    bank.A_BI_all[: ] = sum(interbank.A_BI .* interbankList, dims = 2)
    pass

"汇总各银行之非银行间资产``A_{-BI}``。"
def together_B_A_exBI(bank:BankCommercial, bankList:TypeState{1}):
...
"汇总各银行之总负债。"
def together_B_Z_all(bank:BankCommercial, bankList:TypeState{1}):
    bank.Z_all[bankList] = bank.Z_BI_all[bankList] + bank.Z_exBI[bankList]
    pass

"加总各银行之银行间总负债，通过银行间负债邻接矩阵。"
def sum_B_Z_BI(bank:BankCommercial, interbank:BankInterbank, bankList:TypeState{1}, interbankList:TypeState{2}):
    bank.Z_BI_all[: ] = sum(interbank.Z_BI .* interbankList, dims = 2)
    pass

"汇总各银行之非银行间负债``Z_{-BI}``。"
def together_B_Z_exBI(bank:BankCommercial, bankList:TypeState{1}):
    bank.Z_exBI[bankList] = bank.Z_D[bankList] + bank.Z_other[bankList]
    pass

"计算各银行之所有者权益``E_{B}``，通过总资产与总负债差值。"
def calc_B_E_all(bank:BankCommercial, bankList:TypeState{1}):
    bank.E_all[bankList] = max.(bank.A_all[bankList] - bank.Z_all[bankList] - LESS1[bankList], 0.0)
    pass

"计算银行体系内包括已退出银行在内的所有各银行之所有者权益``E_{B}``，通过总资产与总负债差值。"
def calc_B_E_all_at_all_bank(bank:BankCommercial, bankList:TypeState{1}):
    bank.E_all[: ] = max.(bank.A_all - bank.Z_all - LESS1, 0.0)
    pass


"计算各银行之总资产``A_{B}``，通过所有者权益和总负债。"
def calc_B_A_all(bank:BankCommercial, bankList:TypeState{1}):
    bank.A_all[bankList] = bank.E_all[bankList] + bank.Z_all[bankList]
    pass

"计算各银行之总负债``Z_{B}``，通过所有者权益和总资产。"
def calc_B_Z_all(bank:BankCommercial, bankList:TypeState{1}):
...
"""
def update_BI_balanceSheet(interbank:BankInterbank; byWay:String):
    if byWay == "Z_BI"
        interbank.A_BI = interbank.Z_BI'
    elif byWay == "A_BI"
        interbank.Z_BI = interbank.A_BI'
...
"""
def update_B_balanceSheet(bank:BankCommercial, interbank:BankInterbank, bankList:TypeState{1}, interbankList:TypeState{2}; byWay:String):
    if byWay == "all"
