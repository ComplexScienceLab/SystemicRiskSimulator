...
"汇总各银行之总资产。"
function together_B_A_all!(bank::BankCommercial, bankList::TypeState{1})
...
"汇总各银行之非银行间资产``A_{-BI}``。"
function together_B_A_exBI!(bank::BankCommercial, bankList::TypeState{1})
...
"汇总各银行之总负债。"
function together_B_Z_all!(bank::BankCommercial, bankList::TypeState{1})
...
"汇总各银行之非银行间负债``Z_{-BI}``。"
function together_B_Z_exBI!(bank::BankCommercial, bankList::TypeState{1})
    bank.Z_exBI[bankList] = bank.Z_D[bankList] + bank.Z_other[bankList]
end

"计算各银行之所有者权益``E_{B}``，通过总资产与总负债差值。"
function calc_B_E_all!(bank::BankCommercial, bankList::TypeState{1})
...
function calc_B_A_all!(bank::BankCommercial, bankList::TypeState{1})
    bank.A_all[bankList] = bank.E_all[bankList] + bank.Z_all[bankList]
end

"计算各银行之总负债``Z_{B}``，通过所有者权益和总资产。"
function calc_B_Z_all!(bank::BankCommercial, bankList::TypeState{1})
    bank.Z_all[bankList] = bank.A_all[bankList] - bank.E_all[bankList]
end

"转换银行间负债为资产。"
function alter_Z_BI!(interbank::BankInterbank)
    interbank.A_BI = interbank.Z_BI'
end

"转换银行间资产为负债。"
function alter_A_BI!(interbank::BankInterbank)
...
function update_B_balanceSheet!(bank::BankCommercial, interbank::BankInterbank, bankList::TypeState{1}, interbankList::TypeState{2}; byWay::String)
    ...
    elseif byWay == "sum Z_BI"
        ...
        together_B_Z_all!(bank, bankList)
        # calc_B_E_all!(bank,bankList) #HACK 冗余。不能调用，只能在外部手动计算。此处可以删除。
    elseif byWay == "alter to Z_BI from A_BI"
        ...
        # calc_B_E_all!(bank,bankList) #HACK 冗余。不能调用，只能在外部手动计算。此处可以删除。
    elseif byWay == "alter to A_BI from Z_BI"
        # sum_B_Z_BI!(bank, interbank,bankList,interbankList) #HACK 冗余。不能调用，只能在外部手动计算。此处可以删除。
        # together_B_Z_all!(bank,bankList) #HACK 冗余。不能调用，只能在外部手动计算。此处可以删除。
