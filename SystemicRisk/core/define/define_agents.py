...
class BankCommercial:
    ...
    def __init__(self):
        ...
        self.A_all = np.np.zeros((env['num_bank'])),  # 总资产 A_all: $A_all=A_BI+A_exBI$
        self.A_BI_all = np.np.zeros((env['num_bank'])),  # 银行间资产加总 A_BI_all
        self.A_exBI = np.np.zeros((env['num_bank'])),  # 非银行间资产 A_exBI: $A_exBI=A_P+A_Q+A_R+A_other$
        self.A_P = np.np.zeros((env['num_bank'])),  # 银行贷款给生产部门之资产（非流动性资产） A_P
        self.A_Q = np.np.zeros((env['num_bank'])),  # 银行持有超额准备金（流动性资产） A_Q
        self.A_R = np.np.zeros((env['num_bank'])),  # 银行持有法定准备金（非流动性资产） A_R
        self.A_other = np.np.zeros((env['num_bank'])),  # 银行持有的其它资产（非流动性资产） A_other
        self.Z_all = np.np.zeros((env['num_bank'])),  # 总负债 Z_all: $Z_total=Z_BI+Z_exBI$
        self.Z_BI_all = np.np.zeros((env['num_bank'])),  # 银行间负债加总 Z_BI_all
        self.Z_exBI = np.np.zeros((env['num_bank'])),  # 非银行间负债 Z_exBI: $Z_exBI=Z_D+Z_other$
        self.Z_D = np.np.zeros((env['num_bank'])),  # 银行获得居民部门存款（非流动性负债） Z_D
        self.Z_other = np.np.zeros((env['num_bank'])),  # 银行持有的其他负债（非流动性负债） Z_other
        self.E_all = np.np.zeros((env['num_bank'])),  # 所有者权益 E_all
        self.T_all = np.np.zeros((env['num_bank'])),  # 总贷款流出 Lo_all: $Lo_all=Lo_BI_all+Lo_exBI$
        self.Lo_all = np.np.zeros((env['num_bank'])),  # 银行间贷款流出 Lo_BI_all
        self.Lo_BI_all = np.np.zeros((env['num_bank'])),  # 非银行间贷款流出 Lo_exBI: $Lo_exBI=Lo_P$
        self.Lo_exBI = np.np.zeros((env['num_bank'])),  # 银行贷款流出给生产部门 Lo_P
        self.Lo_P = np.np.zeros((env['num_bank'])),  # 总交易流量 Transfer_all: $Transfer_all=Lo_all+Li_all+Bi_all+Bo_all$
        self.Li_all = np.np.zeros((env['num_bank'])),  # 总贷款流入 Li_all: $Li_all=Li_BI_all+Li_exBI$
        self.Li_BI_all = np.np.zeros((env['num_bank'])),  # 银行间贷款流入 Li_BI_all
        self.Li_exBI = np.np.zeros((env['num_bank'])),  # 非银行间贷款流入 Li_exBI: $Li_exBI=Li_D$
        self.Li_P = np.np.zeros((env['num_bank'])),  # 银行贷款流入从生产部门 Li_P
        self.Bi_all = np.np.zeros((env['num_bank'])),  # 总借款流入 Bi_all: $Bi_all=Bi_BI_all+Bi_exBI$
        self.Bi_BI_all = np.np.zeros((env['num_bank'])),  # 银行间借款流入 Bi_BI_all
        self.Bi_exBI = np.np.zeros((env['num_bank'])),  # 非银行间借款流入 Bi_exBI: $Bi_exBI=Bi_D$
        self.Bi_D = np.np.zeros((env['num_bank'])),  # 银行借款流入从居民部门 Bi_D
        self.Bo_all = np.np.zeros((env['num_bank'])),  # 总借款流出 Bo_all: $Bo_all=Bo_BI_all+Bo_exBI$
        self.Bo_BI_all = np.np.zeros((env['num_bank'])),  # 银行间借款流出 Bo_BI_all
        self.Bo_exBI = np.np.zeros((env['num_bank'])),  # 非银行间借款流出 Bo_exBI: $Bo_exBI=Bo_P$
        self.Bo_D = np.np.zeros((env['num_bank'])),  # 银行借款流出给居民部门 Bo_D
        self.Shock_t = np.np.zeros((env['num_bank'])),  # 总冲击目标 Shock_t $Shock_t = Shock_exBI_t+Shock_BI_t$
        self.Shock_s = np.np.zeros((env['num_bank'])),  # 总冲击源头 Shock_s $Shock_s = Shock_exBI_s+Shock_BI_s$
        self.Shock_def_t = np.np.zeros((env['num_bank'])),  # 总违约损失冲击目标 Shock_def_t $Shock_def_t = Shock_exBI_def_t+Shock_BI_def_t$
        self.Shock_def_s = np.np.zeros((env['num_bank'])),  # 总违约损失冲击源头 Shock_def_s $Shock_def_s = Shock_exBI_def_s+Shock_BI_def_s$
        self.Shock_run_t = np.np.zeros((env['num_bank'])),  # 总挤兑流动冲击目标 Shock_run_t $Shock_run_t = Shock_exBI_run_t+Shock_BI_run_t$
        self.Shock_run_s = np.np.zeros((env['num_bank'])),  # 总挤兑流动冲击源头 Shock_run_s $Shock_run_s = Shock_exBI_run_s+Shock_BI_run_s$
        self.Shock_exBI_t = np.np.zeros((env['num_bank'])),  # 非银行间借贷冲击目标 Shock_exBI_t $Shock_exBI_t = Shock_P_def_t+Shock_D_run_t$
        self.Shock_exBI_s = np.np.zeros((env['num_bank'])),  # 非银行间借贷冲击源头 Shock_exBI_s $Shock_exBI_s = Shock_P_run_s+Shock_D_def_s$
        self.Shock_P_run_s = np.np.zeros((env['num_bank'])),  # 银行之厂商贷款挤兑流动冲击源头 Shock_P_run_s
        self.Shock_P_def_t = np.np.zeros((env['num_bank'])),  # 银行之厂商贷款违约损失冲击目标 Shock_P_def_t
        self.Shock_D_def_s = np.np.zeros((env['num_bank'])),  # 银行存款违约损失冲击源头 Shock_D_def_s
        self.Shock_D_run_t = np.np.zeros((env['num_bank'])),  # 银行存款挤兑流动冲击目标 Shock_D_run_t
        self.Shock_B = np.np.zeros((env['num_bank'])),  # 银行内资产负债冲击 Shock_B $Shock_B=Shock_B_A+Shock_B_Z$
        self.Shock_B_A = np.np.zeros((env['num_bank'])),  # 银行内资产负债之银行间资产端冲击 Shock_B_A
        self.Shock_B_Z = np.np.zeros((env['num_bank'])),  # 银行内资产负债之银行间负债端冲击 Shock_B_Z
        self.Shock_BI_s = np.np.zeros((env['num_bank'])),  # 银行间冲击源头 Shock_BI_s $Shock_BI_s=Shock_BI_def_s+Shock_BI_run_s$
        self.Shock_BI_t = np.np.zeros((env['num_bank'])),  # 银行间冲击目标 Shock_BI_t $Shock_BI_t=Shock_BI_def_t+Shock_BI_run_t$
        self.Shock_BI_def_s = np.np.zeros((env['num_bank'])),  # 银行间违约损失冲击源头 Shock_BI_def_s
        self.Shock_BI_def_t = np.np.zeros((env['num_bank'])),  # 银行间违约损失冲击目标 Shock_BI_def_t
        self.Shock_BI_run_s = np.np.zeros((env['num_bank'])),  # 银行间挤兑流动冲击源头 Shock_BI_run_s $Shock_BI_run_s=Shock_BI_run_ilq_s+Shock_BI_run_br_s$
        self.Shock_BI_run_t = np.np.zeros((env['num_bank'])),  # 银行间挤兑流动冲击目标 Shock_BI_run_t $Shock_BI_run_t+Shock_BI_run_ilq_t+Shock_BI_run_br_t$
        self.Shock_BI_run_ilq_s = np.np.zeros((env['num_bank'])),  # 银行间流动性短缺挤兑流动冲击源头 Shock_BI_run_ilq_s
        self.Shock_BI_run_ilq_t = np.np.zeros((env['num_bank'])),  # 银行间流动性短缺挤兑流动冲击目标 Shock_BI_run_ilq_t
        self.Shock_BI_run_br_s = np.np.zeros((env['num_bank'])),  # 银行间倒闭挤兑流动冲击源头 Shock_BI_run_br_s
        self.Shock_BI_run_br_t = np.np.zeros((env['num_bank'])),  # 银行间倒闭挤兑流动冲击目标 Shock_BI_run_br_t
        self.Loss_BI = np.np.zeros((env['num_bank'])),  # 银行间市场冲击损失 Loss_BI
        self.Loss_BI_def_t = np.np.zeros((env['num_bank'])),  # 银行间资产负债违约冲击损失 Loss_BI_def_t
        self.Loss_BI_run_t = np.np.zeros((env['num_bank'])),  # 银行间负债流动性挤兑冲击损失 Loss_BI_run_t
...
class BankInterbank:
    ...
    def __init__(self):
        ...
        self.abbr = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间资产邻接矩阵 A_BI
        self.name = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间负债邻接矩阵 Z_BI
        self.A_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间贷款流出邻接矩阵 Lo_BI
        self.Z_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间贷款流入邻接矩阵 Li_BI
        self.Lo_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间借款流入邻接矩阵 Bo_BI
        self.Li_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间借款流出邻接矩阵 Bi_BI
        self.Bo_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间冲击 Shock_BI: $Shock_BI=Shock_BI_def+Shock_BI_run$
        self.Bi_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间违约损失冲击 Shock_BI_def
        self.Shock_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间挤兑流动冲击 Shock_BI_run: $Shock_BI_run=Shock_BI_run_ilq+Shock_BI_run_br$
        self.Shock_BI_def = np.np.zeros((self, env['num_bank'], env['num_bank'])),  # 流动性短缺银行银行间挤兑流动冲击 Shock_BI_run_ilq
        self.Shock_BI_run = np.np.zeros((env['num_bank'], env['num_bank'])),  # 破产银行银行间挤兑流动冲击 Shock_BI_run_br
        self.Shock_BI_run_ilq = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间市场冲击损失 Loss_BI
        self.Shock_BI_run_br = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间资产负债违约冲击损失 Loss_BI_def
        self.Loss_BI = np.np.zeros((env['num_bank'], env['num_bank'])),  # 银行间负债流动性挤兑冲击损失 Loss_BI_run
        self.Loss_BI_def = np.np.zeros((self, env['num_bank'], env['num_bank'])),  # 信息邻接矩阵之于是否有银行间敞口 isExposure
