...
class ModelSetter:
    ...
    def initVariables_setManually(self, self):
        ...
        interbank = BankInterbank(
            ...
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间借款流出邻接矩阵 Bi_BI
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间冲击 Shock_BI: $Shock_BI=Shock_BI_def+Shock_BI_run$:
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间违约损失冲击 Shock_BI_def:
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间挤兑流动冲击 Shock_BI_run: $Shock_BI_run=Shock_BI_run_ilq+Shock_BI_run_br$
            np.np.zeros((env['num_bank'], env['num_bank'])), # 流动性短缺银行银行间挤兑流动冲击 Shock_BI_run_ilq
            np.np.zeros((env['num_bank'], env['num_bank'])), # 破产银行银行间挤兑流动冲击 Shock_BI_run_br
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间市场冲击损失 Loss_BI
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间资产负债违约冲击损失 Loss_BI_def:
            np.np.zeros((env['num_bank'], env['num_bank'])), # 银行间负债流动性挤兑冲击损失 Loss_BI_run
            # np.np.zeros((env['num_bank'], env['num_bank'])), # 信息邻接矩阵之于是否有银行间敞口 isExposure
