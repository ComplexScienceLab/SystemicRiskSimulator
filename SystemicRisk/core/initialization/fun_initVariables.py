...
from SystemicRisk.core import ModelComponent,ModelContent
from SystemicRisk.model.models.model_content import *

class ModelVariableSetter:
    """
    设置模型及其变量
    """
    ...
    def init_B_variables_only(self):
        ...
        bank = BankCommercial{1,1}(
            ...
            np.full(env['num_bank'],""), # 缩写 abbr
            np.full(env['num_bank'],""), # 全名 name
        ...
        interbank = BankInterbank{2}(
            ...
            np.zeros((env['num_bank'], env['num_bank'])), # 银行间资产邻接矩阵 A_BI
            np.zeros((env['num_bank'], env['num_bank'])), # 银行间负债邻接矩阵 Z_BI
