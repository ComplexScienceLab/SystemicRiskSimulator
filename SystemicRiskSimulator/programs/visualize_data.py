...
def visualize_data(sgv:dict):
    ...
    if (sgv['visulization_process']['导入Pandas格式的实验结果数据转换为面板形式再导出']):
        ...
        for filepath_pkl_BB in list_filepath_pkl_BB:
            ...
            filepath_pkl_BB_panal = Path(sgv['folderpath_experiments_output_data'], filename_pkl_BB_panel)  # 面板数据文件路径
        ...
        for filepath_pkl_IB in list_filepath_pkl_IB:
            ...
            filepath_pkl_IB_panal = Path(sgv['folderpath_experiments_output_data'], filename_pkl_IB_panel)  # 面板数据文件路径
    ...
    if (sgv['visulization_process']['导入面板形式的CSV数据预处理']):
        ...
        list_filepath_csv_panel = glob.glob(str(sgv['folderpath_experiments_output_data'] / f'*.csv'))  # 获取实验组输出数据csv格式的文件列表
        ...
        for i_exp in experiments_indices:
            ...
            num_data_class = len(sgv['list_data_name'])  # 数据类别数
            ## 根据时间粒度参数，确定时间轴名称及其长度
            if sgv['time_granularity'] == '步进粒度':
                name_time = 'id_data'
                num_time = num_step
            elif sgv['time_granularity'] == '轮次粒度':
                name_time = 'round'
                num_time = num_round
            else:
    ...
    if (sgv['visulization_process']['读取面板形式的PKL格式的文件']):
        ...
        pkl_panel_file_list = glob.glob(str(sgv['folderpath_experiments_output_data'] / '*_panel_*.pkl'))  # 获取实验组输出数据pkl格式之文件列表
    ...
    for i_exp in experiments_indices:
        pkl_BB_00 = pd.read_pickle(Path(sgv['folderpath_experiments_output_data'], 'BB_panel_exp=' + str(i_exp) + '.pkl'))
        pkl_IB_00 = pd.read_pickle(Path(sgv['folderpath_experiments_output_data'], 'IB_panel_exp=' + str(i_exp) + '.pkl'))
        ...
        if (sgv['visulization_process']['绘制资产负债表']):
            ...
            sgv['vis']['max_value_BB'] = 0  # 获取资产负债数据之最大值
            ...
            sgv['vis']['time_granularity'] = sgv['time_granularity']  # 时间线粒度参数
            sgv['vis']['name_time'] = name_time  # 时间轴名称
            for i in range(num_items_in_a_time_in_BB):
                for t in range(num_time):
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_all'].values[0] > sgv['vis']['max_value_BB']:
                        sgv['vis']['max_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_all'].values[0]
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_all'].values[0] > sgv['vis']['max_value_BB']:
                        sgv['vis']['max_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_all'].values[0]
                        pass  # if
                    pass  # for
                pass  # for
            ...
            for i in range(num_items_in_a_time_in_BB):
                for t in range(num_time):
                    ## 生成资产负债表账户数据
                    one_bank_accounts = get_one_bank_accounts_data(df_BB_panel, t, i, sgv['vis'])
                    ## 生成相关的参数
                    sgv['vis']['bank_name'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['name'].values[0]
                    sgv['vis']['process_name'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['process_name'].values[0]
                    sgv['vis']['step'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['step'].values[0]
                    sgv['vis']['round'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['round'].values[0]
                    sgv['vis']['phase'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['phase'].values[0]
                    ## 绘制资产负债表
                    one_bank_balanceSheet_svg = draw_one_bank_BalanceSheet(one_bank_accounts, sgv['vis'], width=500, height=500, title_height=15, border=20)
                    ## 保存结果
                    one_bank_balanceSheet_svg.save_svg(Path(sgv['folderpath_plots_single_balanceSheets'], 'BB_exp=' + str(i_exp) + '+name=' + sgv['vis']['bank_name'] + '+' + name_time + '=' + str(t) + '.svg'))
                    pass  # for
                pass  # for
        ...
        if (sgv['visulization_process']['绘制资金流网络图']):
            ...
            sgv['vis']['max_value_BB'] = 0
            sgv['vis']['min_value_BB'] = float('inf')
            for i in range(num_items_in_a_time_in_BB):
                for t in range(num_time):
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_IB_all'].values[0] > sgv['vis']['max_value_BB']:
                        sgv['vis']['max_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_IB_all'].values[0]
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_IB_all'].values[0] < sgv['vis']['min_value_BB']:
                        sgv['vis']['min_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['A_IB_all'].values[0]
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_IB_all'].values[0] > sgv['vis']['max_value_BB']:
                        sgv['vis']['max_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_IB_all'].values[0]
                    if df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_IB_all'].values[0] < sgv['vis']['min_value_BB']:
                        sgv['vis']['min_value_BB'] = df_BB_panel[(df_BB_panel[name_time] == t) & (df_BB_panel['id_agent'] == i)]['Z_IB_all'].values[0]
                        pass  # if
                    pass  # for
                pass  # for

            ## 获取各资金流数据之流量数据之最大值和最小值
            sgv['vis']['max_value_IB'] = 0
            sgv['vis']['min_value_IB'] = float('inf')
            for i in range(num_items_in_a_time_in_IB):
                for t in range(num_time):
                    if df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['A_IB'].values[0] > sgv['vis']['max_value_IB']:
                        sgv['vis']['max_value_IB'] = df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['A_IB'].values[0]
                    if df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['A_IB'].values[0] < sgv['vis']['min_value_IB']:
                        sgv['vis']['min_value_IB'] = df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['A_IB'].values[0]
                    if df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['Z_IB'].values[0] > sgv['vis']['max_value_IB']:
                        sgv['vis']['max_value_IB'] = df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['Z_IB'].values[0]
                    if df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['Z_IB'].values[0] < sgv['vis']['min_value_IB']:
                        sgv['vis']['min_value_IB'] = df_IB_panel[(df_IB_panel[name_time] == t) & (df_IB_panel['id_agent'] == i)]['Z_IB'].values[0]
                        pass  # if
                    pass  # for
                pass  # for

            ## 绘图
            for data_name in sgv['list_data_name']:
                for t in range(num_time):
                    ## 生成相关的参数
                    sgv['vis']['data_name'] = data_name
                    sgv['vis']['process_name'] = df_BB_panel[df_BB_panel[name_time] == t]['process_name'].values[0]
                    sgv['vis']['step'] = df_BB_panel[df_BB_panel[name_time] == t]['step'].values[0]
                    sgv['vis']['round'] = df_BB_panel[df_BB_panel[name_time] == t]['round'].values[0]
                    sgv['vis']['phase'] = df_BB_panel[df_BB_panel[name_time] == t]['phase'].values[0]
                    ## 获取相关的节点与边信息
                    graph_data = get_graph_data_info(t, df_BB_panel, df_IB_panel, sgv['vis'])
                    ## 用igraph绘制
                    graph_fig = draw_interbank_flow_graph(graph_data, sgv['vis'])
                    ## 保存
                    graph_fig.savefig(Path(sgv['folderpath_plots_single_graphs'], 'IB_exp=' + str(i_exp) + '+data=' + data_name + '+' + name_time + '=' + str(t) + '.pdf'))
                    pass  # for
                pass  # for
