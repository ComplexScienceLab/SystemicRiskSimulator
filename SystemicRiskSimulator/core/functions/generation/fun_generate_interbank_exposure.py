...
def generate_interbank_exposure(A_IB, Z_IB, target_density, n_samples_calib=10, thin=100, folderpath_result: Path = None):
    """
    ...
    #
    # # 运行 R 脚本
    # # os.system(f"Rscript fun_generate_interbank_exposure.R -t {target_density} -i {folderpath_result} -o {Path(folderpath_result, 'reconstructed_L.csv')} -n {n_samples_calib} -l {thin}")
    # os.system(rf'Rscript {Path(Path(__file__).parent.resolve(), "fun_generate_interbank_exposure.R")} -d {target_density} -i "{str(folderpath_result)}" -o "{Path(folderpath_result, "reconstructed_L.csv")}" -n {n_samples_calib} -l {thin}')
    #
    # # 读取生成的 CSV 文件
