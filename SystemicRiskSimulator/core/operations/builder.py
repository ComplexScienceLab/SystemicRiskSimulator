...
class Builder:
    ...
    def build_entities_by_process_and_container_component(cls, sgv: SimulatorGlobalVariableType = sgv):
        ...
        for mainModelInstanceEntity in EntityManager.mainModelTemplateEntities.values():
            mainModelInstanceEntity.attribute.other['process_state'] = "has not process"  # 设置节点之处理状态 #DEBUG 这个似乎无用。
        ...
        for modelEntity in EntityManager.modelEntities.values():
            if (
                ...
                if modelEntity.execute is not None:
                    modelEntity.execute = eval(modelEntity.execute)  # 设置执行器之值是处理机 #HACK 是否有必要继续用处理机处理？
                    pass  # if
                pass  # if
            if (
                    modelEntity.attribute.entity_type == {"template entity"} and
