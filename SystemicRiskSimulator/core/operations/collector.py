...
from SystemicRiskSimulator import np, pd, deepcopy, path, Optional, logging
...
class Collector:
    ...
    def export_agent_data(cls, A_data: AgentDataCollection, env: dict):
        ...
        #     )
        # ]  # 获取需要展平的列
        # list_columns_for_explode = A_data.BB.columns  # 获取需要展平的列
        #
        # BB_data_export = A_data.BB.explode(list_columns_for_explode)  # 展平，面板化数据框
        # BB_data_export(path.join(env['folderpath_of_experiments_output_data'], "BB_exp=" + str(env['id_experiment']) + ".csv"))  # 导出为csv格式；
        #
        # ## 整理interbank之数据为一数据框
