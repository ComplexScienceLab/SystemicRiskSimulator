...
function initAgentDataCollection(A::SystemicRiskAgent)
    ...
    dict_fields = merge(Dict([(:tau, ones(env[:numBank])), (:indexStage, ones(env[:numBank]))]),struct2dict(A.BB))
...
function collector(A::SystemicRiskAgent, A_data::AgentDataCollection; env::Dict=env)
    ...
    # data_BB[!, :id] = collect(range(1, env[:numBank], step=1))
    data_BB[!, :tau] = fill(env[:tau], env[:numBank])
    data_BB[!, :indexStage] = fill(env[:indexStage], env[:numBank])
