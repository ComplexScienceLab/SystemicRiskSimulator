...
function model_BI1111(BB::BankCommercial, BI::BankInterbank, para::Dict, env::Dict)
    ...
    # run_process(env[:processName], :(BB, BI, BB_t1, BI_t1, env = process_exBank_insolvent!(BB, BI, para, env)))

    ## 过程：资不抵债银行间违约损失传染冲击
    BB, BI, BB_t1, BI_t1, env = process_interBank_insolvent!(BB, BI, BB_t1, BI_t1, para, env)
    # run_process(env[:processName], :(BB, BI, BB_t1, BI_t1, env = process_interBank_insolvent!(BB, BI, BB_t1, BI_t1, para, env)))

    ## 过程：外部挤兑流动传染冲击
    # run_process(env[:processName], :(BB, BI, env = process_exBank_illiquity!(BB, BI, para, env)))

    ## 过程：流动性短缺银行间挤兑流动传染冲击
    # run_process(env[:processName], :(BB, BI, env = process_interBank_illiquity!(BB, BI, para, env)))

    ## 过程：外生破产银行间挤兑流动传染冲击 #HACK暂时不用
    # run_process(env[:processName],:(BB, BI, env = process_exBank_bankrupt!(BB, BI,para,env)))

    ## 过程：破产银行间挤兑流动传染冲击
    # run_process(env[:processName], :(BB, BI, env = process_interBank_bankrupt!(BB, BI, para, env)))
