...
"函数：银行外部违约损失冲击阶段"
function stage_exBank_insolvent_shock!(BB::BankCommercial, BI::BankInterbank, b::TypeState{1}, ib::TypeState{2}, para::Dict, env::Dict)
    ## # 银行外部违约损失冲击阶段
    ...
    BB.Shock_P_def_t[BB.isv] = zeros(env[:num_bank])[BB.isv] # 清零银行间和银行外冲击变量
