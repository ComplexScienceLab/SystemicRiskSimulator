...
# "函数：外生破产银行间挤兑流动冲击"#HACK无用
# function exBank_bankrupt_shock!(BB::BankCommercial, BI::BankInterbank, b::TypeState{1}, ib::TypeState{2}, para::ParameterVariables)
#     ## # 外生破产银行银行间挤兑流动传染冲击
#     if BB_t0.br != FALSE1 # 当最初存在已经判定倒闭的银行时执行以下过程
...
function bankrupt_repay_shock!(BB::BankCommercial, BI::BankInterbank, b::TypeState{1}, ib::TypeState{2}, para::ParameterVariables)
    ...
    # BB.Shock_BI_run_br_t[BB.br] = BB.Z_BI_all[BB.br] # 计算破产银行遭受偿还银行间负债冲击
    # for i in findall(BB.br) # 破产银行遭受偿还银行间负债冲击
    #     BI.Shock_BI_run_br[i,BB.cre[i]] = BI.A_BI[i, BI.deb[i]]
    # end
    # update_B_Shock!(BB, BI, b, ib; byWay = "Shock_BI_run_br")
...
# "函数：破产银行挤兑流动分配借贷流量阶段" #HACK等同于函数：流动性短缺银行间挤兑流动分配借贷流量阶段
# function interBank_bankrupt_allocate!(BB::BankCommercial, BI::BankInterbank, b::TypeState{1}, ib::TypeState{2}, para::ParameterVariables)
#     ## # 破产银行间挤兑流动分配借贷流量阶段
...
# "函数：破产银行挤兑流动执行借贷流量阶段" #HACK等同于函数：流动性短缺银行间挤兑流动执行借贷流量阶段
# function interBank_bankrupt_repay!(BB::BankCommercial, BI::BankInterbank, b::TypeState{1}, ib::TypeState{2}, para::ParameterVariables)
#     ## # 流动性短缺银行间挤兑流动执行借贷流量阶段
