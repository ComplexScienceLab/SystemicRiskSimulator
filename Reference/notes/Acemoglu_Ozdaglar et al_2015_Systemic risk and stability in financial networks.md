# Acemoglu D, Ozdaglar A, Tahbaz-Salehi A, 2015. Systemic risk and stability in financial networks[J]. Social ence Electronic Publishing, 105(2): 564-608(45).



[toc]



# Systemic risk and stability in financial networks

## Metadata

* Type: [[Article]]
* Authors: [[Daron Acemoglu]], [[Asuman Ozdaglar]], [[Alireza Tahbaz-Salehi]]
* Date: [[2015]]
* Date added: [[2020-08-02]]
* Publication: [[Social ence Electronic Publishing]]
* Cite key: Acemoglu_2015_Systemicriskandstabilityinfinancialnetworks
* Topics: [[系统性风险 影响因素]]
* Related: 
* Tags: [[Zotero Import]]
* PDF Attachments: [Acemoglu_Ozdaglar et al_2015_Systemic risk and stability in financial networks.pdf](zotero://open-pdf/library/items/ILHQR2E2)



### Abstract

 We provide a framework for studying the relationship between the financial network architecture and the likelihood of systemic failures due to contagion of counterparty risk. We show that financial contagion exhibits a form of phase transition as interbank connections increase: as long as the magnitude and the number of negative shocks affecting financial institutions are sufficiently small, more “complete” interbank claims enhance the stability of the system. However, beyond a certain point, such interconnections start to serve as a mechanism for propagation of shocks and lead to a more fragile financial system. We also show that, under natural contracting assumptions, financial networks that emerge in equilibrium may be socially inefficient due to the presence of a network externality: even though banks take the effects of their lending, risk-taking and failure on their immediate creditors into account, they do not internalize the consequences of their actions on the rest of the network.



## Classification

[[系统性风险]]

[[复杂网络方法]]

[[网络体系结构影响因素]]



## Tags

#系统性风险# #复杂网络方法# #网络体系结构影响因素#





## 摘要

我们提供了一个框架，用于研究金融网络体系结构与因交易对手风险的传染而导致系统性故障的可能性之间的关系。我们表明，随着银行间联系的增加，金融危机会表现出一种相变的形式：只要影响金融机构的负面冲击的规模和数量足够小，更“完整”的银行间债权就可以增强系统的稳定性。但是，在一定程度上，这种相互联系开始充当传播冲击的机制，并导致金融体系更加脆弱。我们还表明，在自然契约假设下，由于存在网络外部性，处于均衡状态的金融网络可能在社会上效率低下：即使银行考虑到其直接债权人的贷款，冒险和失败的影响，他们不会内部考虑其行为对网络其余部分的影响。



## 研究问题

研究金融网络体系结构与因交易对手风险的传染而导致系统性故障的可能性之间的关系。  

## 研究发现  

- 随着银行间联系的增加，金融危机会表现出一种相变的形式：只要影响金融机构的负面冲击的规模和数量足够小，更“完整”的银行间债权就可以增强系统的稳定性。但是，在一定程度上，这种相互联系开始充当传播冲击的机制，并导致金融体系更加脆弱。  
- 在自然契约假设下，由于存在网络外部性，处于均衡状态的金融网络可能在社会上效率低下：即使银行考虑到其直接债权人的贷款，冒险和失败的影响，他们不会内部考虑其行为对网络其余部分的影响。  