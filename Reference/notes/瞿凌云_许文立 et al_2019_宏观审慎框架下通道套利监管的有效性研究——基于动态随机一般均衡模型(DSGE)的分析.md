...
$$
其中, $i _{ t }$ 为信贷规模; $gdp _{ t }$ 为国内生产总值。 根据资本充足率的规定, 商业银行的自有资本不得低于其贷款的一定比例,即商业银 行的资本充足率约束为 $n _{ t } \geq \bar{\eta} S _{ t ^{\circ}}^{ c }$ 其中, $\bar{\eta}$ 为最低资本充足率要求。本文允许商业银行自
有资本小于监管要求的最低资本。但监管当局会惩罚商业银行,思罚成本是资本缺口的函 数。定义资本缺口 $x _{ t }= n _{ t }-\bar{\eta} S _{ t }^{ C }$, 参照 Fève and Pierrard( 2017) 回 , 本文将惩罚函数设置为
$\left( x _{ t }\right)=- p _{1} \ln \left(1+ p _{2} x _{ t }\right)$, 其中 $p _{1}, p _{2} \geq 0$ 。 这意味着 $c ^{\prime}(0)=- p _{1} p _{2} \leq 0, c ^{\prime \prime}(0)=- c ^{\prime}(0) p _{2}$
$\geq 0$