...
- 目标
  ...
  - 效用$U_F$最大化
    $$
    \max _{\left\{ C _{t}^{ L }, \mu_{t}^{ F }, D _{ t }^{ L }, \mu_{ t }, \mu_{ t }^{ M }\right\}} E _{0}\left\{\sum_{t=0}^{\infty} \beta_{ L }^{ t }\left[\frac{\left( C _{ t }^{ L }\right)^{1-\sigma}}{(1-\sigma)}+\eta \log \left( k _{ t }^{ L }- k ^{o}\right)\right]\right\}
    $$
    
  - 资金流约束（左出右入）（对应拉格朗日乘子$\lambda_t^L$）：
    $$
    \begin{aligned}
          ...
          \end{aligned}
    $$
    股利分红 + 贷款至企业 + 还居民存款 + 还央行贴现的 $\le$ 收回贷款从企业当未违约时 + 收回贷款从银行当未违约时 + 吸收居民存款 + 向央行贴现。
    
    
    
  - 存贷比约束（对应拉格朗日乘子$\lambda_t^{LL}$）：
    $$
  ...
  - 资本充足率定义：
    $$
    k _{ t }^{ L }=\frac{ E _{ t }^{ L }}{\omega_{ t }^{ F } \mu_{ t }^{ F }+\omega_{ t }^{ M } \mu_{ t }^{ M }}
    $$
  ...
  - 银行权益时变公式
    ...
    \end{aligned}
    $$
    银行权益 = 上一期银行权益 + 从企业贷款之收益 + 同业拆借之收益 - 还居民存款 - 还央行贴现 - 股利分红
...
- 行为
  ...
  - 提供消费贷款本金$\mu_t^B$以利率$r_t^B$给居民$B$；
    ...
    - 目标
      ...
      $$
      \max _{\left\{ C _{t}^{ D }, D _{t}^{ D }, \mu_{t}^{1}, \mu_{t}^{ R }, \mu_{t}^{ N }, v_{t}^{ I }\right\}} E _{0}\left\{\sum_{t=0}^{\infty} \beta_{ D }^{ t }\left[\frac{\left( C _{ t }^{ D }\right)^{1-\sigma}}{(1-\sigma)}+\eta \log \left( k _{ t }^{ D }- k ^{o}\right)\right]\right\}
      $$
      
      其中，$\sigma$：银行风险厌恶系数；$\eta$：资本缓冲偏好；$(k^D_t-k^o)$：资本缓冲，是监管要求；
      
    - 资金流约束（左出右入）（对应拉格朗日乘子$\lambda_t^D$）：

      $$
      \begin{array}{l}
      C _{ t }^{ D }+\mu_{ t }^{ R }+\mu_{ t }^{ N }+\frac{ D _{ t -1}^{ D }}{\pi_{ t }}+\nu_{ t }^{ I } \frac{\mu_{ t -1}^{ I }}{\pi_{ t }}+\frac{\theta^{ I }}{2}\left[\left(1-\nu_{ t }^{ I }\right) \frac{\mu_{ t -1}^{ I }}{\pi_{ t }}\right]^{2}\\
      \leqslant \frac{ D _{ t }^{ D }}{1+ r _{ t }^{ S }}+\frac{\mu_{ t }^{ I }}{\left(1+ r _{ t }^{ I }\right)}+{ \nu}_{ t }^{ B } \frac{\mu_{ t -1}^{ R }\left(1+ r _{ t -1}^{ B }\right)}{\pi_{ t }}+\nu_{ t }^{ H } \frac{\mu_{ t -1}^{ N }\left(1+ r _{ t -1}^{ H }\right)}{\pi_{ t }}
      \end{array}
      $$
      股利分红 + 消费贷款给借款型居民 + 贷款住房本金给借款型居民 + 返还储蓄型居民的存款 + 偿还银行间市场拆入本息 + 收益之于银行间市场违约时的惩罚 $\le$ 吸收储蓄型居民的存款 + 同业间市场拆出 + 收回消费贷款从借款型居民 + 收回住房本金贷款从借款型居民。
    
    - 存贷比约束（对应拉格朗日乘子$\lambda_t^{DD}$）：
      $$
      \mu_{ t }^{ R }+\mu_{ t }^{ N } \leqslant \varphi^{ D } \cdot\left( D _{ t }^{ D } / 1+ r _{ t }^{ S }\right)
      $$
      
    - 资本充足率定义：

      $$
      k _{ t }^{ D }=\frac{ E _{ t }^{ D }}{\omega_{ t }^{ R } \mu_{ t }^{ R }+\omega_{ t }^{ N } \mu_{ t }^{ N }}
      $$

    - 银行权益时变公式：
      $$
      E _{ t }^{ D }=\frac{ E _{ t -1}^{ D }}{\pi_{ t }}+\frac{\mu_{ t -1}^{ R }}{\pi_{ t }}\left[\nu_{ t }^{ B }\left(1+ r _{ t -1}^{ B }\right)-1\right]+\frac{\mu_{ t -1}^{ N }}{\pi_{ t }}\left[\nu_{ t }^{ H }\left(1+ r _{ t -1}^{ H }\right)-1\right]- C _{ t }^{ D }\\
      -\frac{ D _{ t -1}^{ D }}{\pi_{ t }\left(1+ r _{ t -1}^{ S }\right)} r _{ t -1}^{ S }-\frac{\mu_{ t -1}^{ I }}{\pi_{ t }\left(1+ r _{ t -1}^{ I }\right)}\left[\nu_{ t }^{ I }\left(1+ r _{ t -1}^{ I }\right)-1\right] \\
      -\frac{\theta^{ I }}{2}\left[\left(1-{\nu}_{ t }^{ I }\right) \frac{\mu_{ t -1}^{ I -1}}{\pi_{ t }}\right]^{2}
      $$
      银行权益 = 上一期银行权益 + 消费贷款从借款型居民之收益 + 房屋贷款从借款型居民之收益 - 股利分红 - 还储蓄型居民存款 - 未违约部分的同业拆借之利息 - 惩罚之于违约还同业拆借
