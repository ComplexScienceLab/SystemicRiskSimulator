...
set_bankCommercialVariables = dict(
    ...
    A_BI_all=np.array([[2185.24, 398.37, 730.99, 1357.75, 2717.39]]).T,  # 银行间资产加总 A_BI_all
    A_exBI=np.zeros((env['num_bank'], 1)),  # 非银行间资产 A_exBI: $A_exBI=A_P+A_Q+A_R+A_other$
    A_P=np.array([[1631.73, 4303.24, 3379.72, 4351.47, 620.69]]).T,  # 银行贷款给生产部门之资产（非流动性资产） A_P
    ...
    Z_BI_all=np.array([[959.6, 1844.24, 1253.34, 2851.85, 480.71]]).T,  # 银行间负债加总 Z_BI_all
    ...
    E_all=np.array([[216.83, 267.13, 233.56, 324.39, 189.66]]).T,  # 所有者权益 E_all
...
set_interbankVariables = dict(
    ...
    cre_isv=np.array([]),  # 信息列表之于资不抵债的银行之债权方银行编号 listOfCreditorsInInsolvent
    deb_isv=np.array([]),  # 信息列表之于资不抵债的银行之债务方银行编号 listOfDebtorsInInsolvent
    cre_ilq=np.array([]),  # 信息列表之于流动性短缺的银行之债权方银行编号 listOfCreditorsInIlliquity
    deb_ilq=np.array([]),  # 信息列表之于流动性短缺的银行之债务方银行编号 listOfDebtorsInIlliquity
    cre_br=np.array([]),  # 信息列表之于破产的银行之债权方银行编号 listOfCreditorsInBankrupt
    deb_br=np.array([]),  # 信息列表之于破产的银行之债务方银行编号 listOfDebtorsInBankrupt
