...
描述银行风险冲击、传染详细见《[描述之风险冲击、损失、传染](../模型描述/描述之风险冲击、损失、传染.md)》。
...
描述银行状态详细见《[描述之银行状态](../建模描述/描述之银行状态.md)》。
...
![模块之银行间市场-模块BI0000传染过程简图.drawio](模块之银行间市场BI0000.assets/模块之银行间市场-模块BI0000传染过程简图.drawio.svg)



详细的银行外冲击传导至银行间过程图：

《 [模块之银行外冲击.drawio.pdf](模块之银行外冲击.drawio.pdf) 》



详细的银行间市场风险传染过程图：

 《 [模块之银行间市场-模块BI0000传染过程](模块之银行间市场-模块BI0000传染过程.drawio.pdf) 》
...
=& \theta_{Shock_{exBI}} \; Shock_{L_{-BI}}[i,t_{1}] + (1-\theta_{Shock_{exBI}}) \; Shock_{D}[i,t_{1}]
...
对于仅考虑非银行间贷款损失外生冲击，暨式$\eqref{银行内资产负债冲击来自银行间市场外之外生冲击}$之$\theta_{Shock_{exBI}}=1$ 。非银行间贷款损失外生冲击影响银行资产端之非银行间贷款$L_{-BI}[i,t_{1}]$，其定义为
...
$$
\begin{align}
Shock_{BI,L_{-BI}}[:,i,\tau_{1}] =& \begin{cases}
0 & ,Shock_{L_{-BI}}[i,t_{1}] \le E_{B}[i,t_{0}] \notag \\
Shock_{L_{-BI}}[i,t_{1}] - E_{B}[i,t_{0}] & ,Shock_{L_{-BI}}[i,t_{1}] \le E_{B}[i,t_{0}]
\end{cases} \\
=& \max\{\min\{Shock_{L_{-BI}}[:,i,t_{1}],A_{-BI}[i,t_{0}]\},0\}
...
$$
\begin{equation}
Shock_{D}[i,t_{1}] = \max\{D_{B}[i,t_{0}],0\} = \max\{Z_{-BI}[i,t_{0}] ,0\}
\label{存款挤兑流动银行外冲击}
\end{equation}
$$
...
- 情况2：当$Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \ge Shock_{D}[i,t_{1}] > Q_{B}[i,t_{0}]$时，银行动用所有流动资金（准备金）$Q_{B}[i,t_{1}]$，仍未能偿还负债。此时银行处于流动性短缺状态$i \in \mathbb{B}_{ilq}$ 。流动性短缺银行$i_{ilq}$会收回银行间资产偿债缓解流动性问题。银行传递剩余外生冲击到银行体系内，成为银行间传染冲击。
- 情况3：当$Shock_{D}[i,t_{1}] > Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}]$时，如果后续传染轮次中，银行$i_{ilq}$全部收回资产仍然不足以偿还剩余负债，则会进入资不抵债状态$i \in \mathbb{B}_{isv}$，根据假设，银行$i_{isv}$进入破产状态$i \in \mathbb{B}_{br}$ 。

由银行$i$之存款挤兑流动银行外冲击传导至银行间市场冲击为
$$
\begin{align}
Shock_{BI,D}[:,i,\tau_{1}] =& \begin{cases}
0 & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \notag \\
Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}] & , Shock_{D}[i,t_{1}] > Q_{B}[i,t_{0}]
\end{cases} \\
=& \max\{\min\{Shock_{D}[i,t_{1}],Z_{-BI}[i,t_{0}]\} - Q_{B}[i,t_{0}],0\}
...
银行间挤兑流动冲击，$Shock_{B,Z_{BI}}[i,t_{1}]$影响银行负债端之银行间负债$Z_{BI}[i,t_{0}]$，其定义为
...
\sum_{i}{Shock_{BI,L_{-BI}}[:,i,\tau]} & , i \in \mathbb{B}_{isv} \wedge t \in \tau = 1\\
\sum_{i}{Shock_{BI,D}[:,i,\tau]} & , i \in \mathbb{B}_{ilq} \cup \mathbb{B}_{br} \wedge t \in \tau = 1\\
\end{cases}
\label{银行间传染冲击在初始轮次}
\end{equation}
$$
非初始轮次冲击$\tau=2,\cdots,N_{\tau}$时，由来式$\eqref{银行间传染冲击通过单个银行银行间贷款损失冲击} \eqref{银行间传染冲击通过单个银行银行间挤兑流动冲击}$可知来自银行$i$之银行间传染冲击为
$$
\begin{equation}
Shock_{BI}[:,i,t_{2}] = \begin{cases}
Shock_{BI,def}[:,i,\tau] & , i \in \mathbb{B}_{isv} \wedge t \in \tau>1\\
Shock_{BI,run}[:,i,\tau] & , i \in \mathbb{B}_{ilq} \cup (\mathbb{B}_{br} \cap \mathbb{B}_{isv}) \wedge t \in \tau>1 \\
\end{cases}
...
我们临时使用系统风险指标SR、脆弱性指标VBI、传染性指标CBI构建测度风险，先看效果。具体描述之于SR指标详见《[描述之系统性风险测度](../模型描述/描述之系统性风险测度.md)》
