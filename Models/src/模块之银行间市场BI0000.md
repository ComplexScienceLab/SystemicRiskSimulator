...
由式$\eqref{非银行间贷款损失银行外冲击}$$\eqref{所有者权益时变之于非银行间贷款损失银行外冲击}$可得银行之非银行间资产变为
...
根据式$\eqref{非银行间贷款损失银行外冲击}$$\eqref{所有者权益时变之于非银行间贷款损失银行外冲击}$$\eqref{非银行间资产时变之于非银行间贷款损失外生冲击}$，银行会有以下情况：
...
$$
\begin{align}
Z_{BI}[i,t_{1}] =& \begin{cases}
Z_{BI}[i,t_{0}] & ,Shock_{L_{-BI}}[i,t_{1}] \le E_{B}[i,t_{0}] \\
Z_{BI}[i,t_{0}] - (Shock_{L_{-BI}}[i,t_{1}] - E_{B}[i,t_{0}]) & , E_{B}[i,t_{0}] < Shock_{L_{-BI}}[i,t_{1}] < E_{B}[i,t_{0}] + Z_{BI}[i,t_{0}] \\
0 & , E_{B}[i,t_{0}] + Z_{BI}[i,t_{0}] \le Shock_{L_{-BI}}[i,t_{1}]
\end{cases}
...
- 情况2：当$Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] $时，银行动用所有流动资金（准备金）$Q_{B}[i,t_{1}]$，仍未能偿还负债。此时银行处于流动性短缺状态$i \in \mathbb{B}_{ilq}$ 。流动性短缺银行$i_{ilq}$会收回银行间资产偿债缓解流动性问题。银行传递剩余外生冲击到银行体系内，成为银行间传染冲击。
...
\begin{align}
A_{BI}[i,t_{1}] =& \begin{cases}
A_{BI}[i,t_{0}] & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \\
A_{BI}[i,t_{0}] - (Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}]) & , Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] < Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \\
0 & , Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \le Shock_{D}[i,t_{1}]
\end{cases} \\
=& \max\{A_{BI}[i,t_{0}],0\} \notag
...
经过收回部分银行间资产并且偿还剩余存款负债之后，重写式$\eqref{非银行间负债时变之于存款挤兑流动银行外冲击当未回收资产时}$可得银行$i$之非银行间负债变动如下：
$$
\begin{align}
Z_{-BI}[i,t_{1}] =& \begin{cases}
Z_{-BI}[i,t_{0}] - Shock_{D}[i,t_{1}] & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \\
Z_{-BI}[i,t_{0}] - Q_{B}[i,t_{0}] - \Delta A_{BI}[i,t_{1}] & , Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \\
Z_{-BI}[i,t_{0}] - Q_{B}[i,t_{0}] - A_{BI}[i,t_{0}] & , Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] < Shock_{D}[i,t_{1}]
\end{cases} \\
=& \max\{Z_{-BI}[i,t_{0}] - Shock_{D}[i,t_{1}],0\} \notag
\label{非银行间负债时变之于存款挤兑流动银行外冲击当回收资产时}
\end{align}
$$
...
NOW 根据式$\eqref{银行间贷款损失冲击}$$\eqref{所有者权益时变之于银行间贷款损失冲击}$，银行会有以下情况：
...
NOW 根据式 $\eqref{银行间挤兑流动冲击}$$\eqref{流动资金时变之于银行间挤兑流动冲击}$，银行会有以下情况：
...
\end{align*}
$$
NOW 因此流动性短缺银行$i_{ilq}$对应单个债务银行$k_{deb}$之资产回收量为
$$
\begin{equation}
...
流动性短缺银行$i_{ilq}$对应单个债务银行$k_{deb}$之负债变动量为：
$$
\begin{align}
