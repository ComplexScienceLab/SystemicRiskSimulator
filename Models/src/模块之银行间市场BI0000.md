...
由于本模块假设负债偿付等级相同，因此由式$\eqref{银行间负债时变之于资不抵债银行违约}$可得资不抵债银行$i_{isv}$对应债权银行$j_{cre}$之负债的实际价值变为
$$
\begin{align}
...
NOW 由式$\eqref{银行间传染冲击通过单个银行非银行间贷款损失外生冲击}$$\eqref{资不抵债银行之单个债权方银行之银行间负债时变之于资不抵债银行违约}$可得债权银行$j_{cre}$遭受单个资不抵债银行$i_{isv}$冲击为
$$
\begin{align}
