...
Z_{BI}[i,t_{0}] & , Shock_{L_{-BI}}[i,t_{1}] > E_{B}[i,t_{0}] + A_{BI}[i,t_{0}]
\end{cases} \\
...
当银行$i_{ilq}$由于流动性短缺时，被迫收回部分银行间资产$Li_{BI}[i_{ilq},t_{1}]=\Delta A_{BI}[i_{ilq},t_{2}]$ ，全部用于补充短缺的流动资金$Q_{B}[i,t_{0}]$，而后该资金立刻被用于偿还剩余的存款挤兑流动银行外冲击$Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}]$。导致其银行间资产负债量变动为
$$
\begin{align}
\Delta A_{BI}[i,t_{1}] =& \begin{cases}
0 & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \notag \\
Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}] & , Shock_{D}[i,t_{1}] > Q_{B}[i,t_{0}] \notag \\
A_{BI}[i,t_{0}] & , Shock_{D}[i,t_{1}] > Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}]
\end{cases} \\
