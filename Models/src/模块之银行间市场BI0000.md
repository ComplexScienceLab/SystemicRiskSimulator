...
根据式$\eqref{银行间贷款损失冲击}$$\eqref{所有者权益时变之于银行间贷款损失冲击}$，银行会有以下情况：
...
由式$\eqref{银行间传染冲击通过单个银行非银行间贷款损失外生冲击}$$\eqref{资不抵债银行之单个债权方银行之银行间负债时变之于资不抵债银行违约}$可得债权银行$j_{cre}$遭受单个资不抵债银行$i_{isv}$冲击为
$$
\begin{equation}
