...
![模块之银行间市场-模块BI0000传染过程简图](模块之银行间市场BI0000.assets/模块之银行间市场-模块BI0000传染过程简图.svg)
...
冲击传导至银行之所有者权益，并影响资产端。根据式$\eqref{银行内资产负债冲击来自银行间市场外之外生冲击}$$\eqref{非银行间贷款损失银行外冲击}$可得所有者权益变为
$$
\begin{align}
...
- 第二种，当$Shock_{L_{-BI}}[i,t_{1}] > E_{B}[i,t_{0}]$时，银行资不抵债，暨$i \in \mathbb{B}_{isv}[t]$。此时，银行无法正常偿还银行间债务，被迫违约，通过银行间违约损失冲击传染方式传递冲击到银行体系内其他银行。

当银行$i$由于资不抵债时，被迫违约。违约导致其银行间负债量变动为
$$
\begin{align}
\Delta Z_{BI}[i,t_{1}] =& \begin{cases}
0 & ,Shock_{L_{-BI}}[i,t_{1}] \le E_{B}[i,t_{0}] \notag \\
Shock_{L_{-BI}}[i,t_{1}] - E_{B}[i,t_{0}] & , Shock_{L_{-BI}}[i,t_{1}] > E_{B}[i,t_{0}] \notag \\
Z_{BI}[i,t_{0}] & , E_{B}[i,t_{0}] < Shock_{L_{-BI}}[i,t_{1}] \le E_{B}[i,t_{0}] + A_{BI}[i,t_{0}]
\end{cases} \\
=& \max\{\min\{Shock_{L_{-BI}}[:,i,t_{1}]-E_{B}[i,t_{0}],Z_{BI}[i,t_{0}]\},0\}
...
$$
\begin{equation}
Shock_{BI,def}[:,i_{isv},t_{1}] = {Shock_{BI,L_{-BI}}[:,i_{isv},\tau]} = \Delta Z_{BI}[i_{isv},t_{1}]
...
对于存款挤兑流动银行外冲击，暨式$\eqref{银行内资产负债冲击来自银行间市场外之外生冲击}$之$\theta_{Shock_{exBI}}=0$ 。存款挤兑流动银行外冲击$Shock_{D}[i,t_{1}]$影响银行负债端之存款$D_{B}[i,t_{0}]$，其定义为

$$
\begin{equation}
Shock_{Z_{-BI}}[i,t_{1}] = Shock_{D}[i,t_{1}] = \max\{D_{B}[i,t_{0}],0\} = \max\{Z_{-BI}[i,t_{0}] ,0\}
\label{存款挤兑流动银行外冲击}
\end{equation}
$$
...
Z_{-BI}[i,t_{1}] 
=& \begin{cases} 
Z_{-BI}[i,t_{0}] - Shock_{D}[i,t_{1}]  & , Shock_{D}[i,t_{1}] < Q_{B}[i,t_{0}] \notag \\
Z_{-BI}[i,t_{0}] - Q_{B}[i,t_{0}] & , Shock_{D}[i,t_{1}] \ge Q_{B}[i,t_{0}]
\end{cases} \\
=& \max \{ Z_{-BI}[i,t_{0}] - \min\{ Shock_{D}[i,t_{1}] , Q_{B}[i,t_{0}] \}, 0 \}
...
由式$\eqref{流动资金时变之于存款挤兑流动银行外冲击}$可得非银行间资产变为
...
- 情况2：当$Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \ge Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] $时，银行动用所有流动资金（准备金）$Q_{B}[i,t_{1}]$，仍未能偿还负债。此时银行处于流动性短缺状态$i \in \mathbb{B}_{ilq}$ 。流动性短缺银行$i_{ilq}$会收回银行间资产偿债缓解流动性问题。银行传递剩余外生冲击到银行体系内，成为银行间传染冲击。
- 情况3：当$Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] < Shock_{D}[i,t_{1}]$时，如果后续传染轮次中，银行$i_{ilq}$全部收回资产仍然不足以偿还剩余负债，则会进入资不抵债状态$i \in \mathbb{B}_{isv}$，根据假设，银行$i_{isv}$进入破产状态$i \in \mathbb{B}_{br}$ 。

当银行$i_{ilq}$由于流动性短缺时，被迫收回部分银行间资产$Li_{BI}[i_{ilq},t_{1}]=\Delta A_{BI}[i_{ilq},t_{2}]$ ，全部用于补充短缺的流动资金$Q_{B}[i,t_{0}]$，而后该资金立刻被用于偿还剩余的存款挤兑流动银行外冲击$Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}]$。导致其银行间资产量变动为
$$
\begin{align}
\Delta A_{BI}[i,t_{1}] =& \begin{cases}
0 & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \notag \\
Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}] & , Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}]  \notag \\
A_{BI}[i,t_{0}] & , Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] < Shock_{D}[i,t_{1}]
\end{cases} \\
=& \max\{\min\{Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}],A_{BI}[i,t_{0}]\},0\}
\label{银行间负债时变之于流动性短缺银行收回资产}
\end{align}
$$
由式$\eqref{银行间负债时变之于流动性短缺银行收回资产}$得银行$i$之存款挤兑流动银行外冲击传导至银行间市场冲击为
$$
\begin{align}
...
经过收回部分银行间资产并且偿还剩余存款负债之后，银行$i$资产负债变动如下：
$$
\begin{align}
\Delta D_{B}[i,t_{1}] =& \begin{cases}
- Shock_{D}[i,t_{1}] & , Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] \notag \\
- Q_{B}[i,t_{0}] - \Delta A_{BI}[i,t_{1}] & , Q_{B}[i,t_{0}] < Shock_{D}[i,t_{1}] \le Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}]  \notag \\
- Q_{B}[i,t_{0}] - A_{BI}[i,t_{0}] & , Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] < Shock_{D}[i,t_{1}]
\end{cases} \\
=& \max\{Shock_{D}[i,t_{1}] - Q_{B}[i,t_{0}],0\}
\label{存款负债时变之于流动性短缺银行收回资产}
\end{align}
$$
...
这里银行资产损失冲击指银行间贷款损失冲击。银行间贷款损失冲击，其影响银行资产端之银行间贷款$A_{BI}[i,t_{1}]$，其定义为

$$
\begin{equation}
Shock_{A_{BI}}[i,t_{1}] = \max\{A_{BI}[i,t_{0}] ,0\}
\label{银行间贷款损失冲击}
\end{equation}
$$
...
\begin{align}
E_{B}[i,t_{1}] =& \begin{cases}
E_{B}[i,t_{0}] - Shock_{A_{BI}}[i,t_{1}] & , Shock_{A_{BI}}[i,t_{1}] < E_{B}[i,t_{0}] \notag \\
0 & , Shock_{B_{BI}}[i,t_{1}] \ge E_{B}[i,t_{0}] \\
\end{cases} \\
=& \max \{E_{B}[i,t_{0}] - Shock_{A_{BI}}[i,t_{1}] , 0\}
...
A_{BI}[i,t_{1}] 
=& \begin{cases} 
A_{BI}[i,t_{0}] - Shock_{A_{BI}}[i,t_{1}]  & , Shock_{A_{BI}}[i,t_{1}] < E_{B}[i,t_{0}] \notag \\
A_{BI}[i,t_{0}] - E_{B}[i,t_{0}] & , Shock_{A_{BI}}[i,t_{1}] \ge E_{B}[i,t_{0}]
\end{cases} \\
=& \max \{ A_{-BI}[i,t_{0}] - \min\{ Shock_{A_{BI}}[i,t_{1}] , E_{B}[i,t_{0}] \}, 0 \}
...
NOW 根据式$\eqref{银行间贷款损失冲击}$$\eqref{所有者权益时变之于银行间贷款损失冲击}$会有以下情况：

- 第一种，当$Shock_{A_{BI}}[i,t_{1}] \le E_{B}[i,t_{0}]$时，银行并未出现资不抵债的情况。此时银行状态为健在，暨$i \notin \mathbb{B}_{isv}[t]$。银行不会传递冲击到银行体系内其他银行；
- 第二种，当$Shock_{A_{BI}}[i,t_{1}] > E_{B}[i,t_{0}]$时，银行资不抵债，暨$i \in \mathbb{B}_{isv}[t]$。此时，银行无法正常偿还银行间债务，被迫违约，通过银行间违约损失冲击传染方式传递冲击到银行体系内其他银行。

由银行$i$之银行间贷款损失冲击传导至银行间传染冲击为
$$
\begin{align}
Shock_{BI,def}[:,i,t_{1}] =& \begin{cases}
0 & ,Shock_{A_{BI}}[i,t_{1}] \le E_{B}[i,t_{0}] \notag \\
Shock_{A_{BI}}[i,t_{1}] - E_{B}[i,t_{0}] & ,Shock_{A_{BI}}[i,t_{1}] \le E_{B}[i,t_{0}]
\end{cases} \\
=& \max\{\min\{Shock_{A_{BI}}[:,i,t_{1}],A_{-BI}[i,t_{0}]\},0\}
...
银行间挤兑流动冲击，$Shock_{Z_{BI}}[i,t_{1}]$影响银行负债端之银行间负债$Z_{BI}[i,t_{0}]$，其定义为

$$
\begin{equation}
Shock_{Z_{BI}}[i,t_{1}] = \max\{Z_{BI}[i,t_{0}] ,0\}
\label{银行间挤兑流动冲击}
\end{equation}
$$
...
\begin{align}
Q_{B}[i,t_{1}] =& \begin{cases}
Q_{B}[i,t_{0}] - Shock_{Z_{BI}}[i,t_{1}] & , Shock_{Z_{BI}}[i,t_{1}] < Q_{B}[i,t_{0}] \notag \\
0 & , Shock_{Z_{BI}}[i,t_{1}] \ge Q_{B}[i,t_{0}] \\
\end{cases} \\
=& \max \{Q_{B}[i,t_{0}] - Shock_{Z_{BI}}[i,t_{1}] , 0\}
...
\begin{align}
A_{BI}[i,t_{1}] =& \begin{cases}
A_{BI}[i,t_{0}] - Shock_{Z_{BI}}[i,t_{1}] & , Shock_{Z_{BI}}[i,t_{1}] < Q_{B}[i,t_{0}] \notag \\
A_{BI}[i,t_{0}] - Q_{B}[i,t_{0}] & , Shock_{Z_{BI}}[i,t_{1}] \ge Q_{B}[i,t_{0}]  \\
\end{cases} \\
=& \max \{ A_{BI}[i,t_{0}] - \min \{ Shock_{Z_{BI}}[i,t_{1}] , Q_{B}[i,t_{0}] \} ,0 \}
...
Z_{BI}[i,t_{1}] 
=& \begin{cases} 
Z_{BI}[i,t_{0}] - Shock_{Z_{BI}}[i,t_{1}]  & , Shock_{Z_{BI}}[i,t_{1}] < Q_{B}[i,t_{0}] \notag \\
Z_{BI}[i,t_{0}] - Q_{B}[i,t_{0}] & , Shock_{Z_{BI}}[i,t_{1}] \ge Q_{B}[i,t_{0}]
\end{cases} \\
=& \max \{ Z_{BI}[i,t_{0}] - \min\{ Shock_{Z_{BI}}[i,t_{1}] , Q_{B}[i,t_{0}] \}, 0 \}
...
NOW 根据式 $\eqref{银行间挤兑流动冲击}$$\eqref{流动资金时变之于银行间挤兑流动冲击}$ 会有以下情况：

- 情况1：当$Shock_{Z_{BI}}[i,t_{1}] \le Q_{B}[i,t_{0}]$时，银行并未出现流动性短缺情况$i \notin \mathbb{B}_{ilq}$ 。银行不会传递冲击到银行体系内；
- 情况2：当$Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}] \ge Shock_{Z_{BI}}[i,t_{1}] > Q_{B}[i,t_{0}]$时，银行动用所有流动资金（准备金）$Q_{B}[i,t_{1}]$，仍未能偿还负债。此时银行处于流动性短缺状态$i \in \mathbb{B}_{ilq}$ 。流动性短缺银行$i_{ilq}$会收回银行间资产偿债缓解流动性问题。银行传递剩余冲击到银行体系内，称为银行间传染冲击。
- 情况3：当$Shock_{Z_{BI}}[i,t_{1}] > Q_{B}[i,t_{0}] + A_{BI}[i,t_{0}]$时，如果后续传染轮次中，银行$i_{ilq}$全部收回资产仍然不足以偿还剩余负债，则会进入资不抵债状态$i \in \mathbb{B}_{isv}$，根据假设，银行$i_{isv}$进入破产状态$i \in \mathbb{B}_{br}$ 。

由银行$i$之银行间挤兑流动冲击传导至银行间市场冲击为
$$
\begin{align}
Shock_{BI,run}[:,i,t_{1}] =& \begin{cases}
0 & , Shock_{Z_{BI}}[i,t_{1}] \le Q_{B}[i,t_{0}] \notag \\
Shock_{Z_{BI}}[i,t_{1}] - Q_{B}[i,t_{0}] & , Shock_{Z_{BI}}[i,t_{1}] > Q_{B}[i,t_{0}]
\end{cases} \\
=& \max\{\min\{Shock_{Z_{BI}}[i,t_{1}],Z_{BI}[i,t_{0}]\} - Q_{B}[i,t_{0}],0\}
...
\end{equation}
$$
非初始轮次冲击$\tau=2,\cdots,N_{\tau}$时，由式$\eqref{银行间传染冲击通过单个银行银行间贷款损失冲击}$$\eqref{银行间传染冲击通过单个银行银行间挤兑流动冲击}$可知来自银行$i$之银行间传染冲击为
$$
\begin{equation}
Shock_{BI}[:,i,t_{2}] = \begin{cases}
Shock_{BI,def}[:,i,t_{1}] & , i \in \mathbb{B}_{isv} \wedge t \in \tau>1\\
Shock_{BI,run}[:,i,t_{1}] & , i \in \mathbb{B}_{ilq} \cup (\mathbb{B}_{br} \cap \mathbb{B}_{isv}) \wedge t \in \tau>1 \\
\end{cases}
...
当银行$i_{isv}$由于资不抵债时，被迫违约，通过银行间违约损失冲击传染方式 $i_{isv} \stackrel{BI,def}{\longrightarrow} j_{cre}$ 传染至对应的债权银行$j_{cre}$ 。此时银行$i_{isv}$之银行间负债$Z_{BI}[i_{isv},t_{0}]$受到冲击$Shock_{BI,def}[i,:,t_{2}]$造成银行间违约损失
...
=& \left| {Z_{BI}[i_{isv},j_{cre},t_{1}]} \; \dfrac{\Delta Z_{B}[i_{isv},t_{2}]}{Z_{B}[i_{isv},t_{1}]} \right| \label{债权银行遭受单个银行间违约损失传染冲击损失在单轮}
\end{align}
$$
...
由于债权银行$j_{cre}$银行间资产收缩，因此由式$\eqref{债权银行遭受银行间违约损失传染冲击在单轮}$可得银行间市场之银行间冲击传导为银行内的资产冲击
$$
\begin{equation}
Shock_{B}[j_{cre},\tau+1] = Shock_{BI,def}[j_{cre},:,t_{2}] = \Delta A_{BI}[j_{cre},t_{2}]
\label{债权银行遭受银行内资产冲击}
\end{equation}
$$
...
\end{align*}
$$
因此流动性短缺银行$i_{ilq}$对应单个债务银行$k_{deb}$之负债变动量为：
$$
\begin{align}
Z_{BI}[k_{deb},i_{ilq},t_{2}] =& Z_{BI}[k_{deb},i_{ilq},t_{1}] - Bo_{BI}[k_{deb},i_{ilq},t_{2}] \notag \\
=& Z_{BI}[k_{deb},i_{ilq},t_{1}] \; \left( 1 - \dfrac{Shock_{BI,run}[:,i_{ilq},t_{2}]}{A_{B}[i_{ilq},t_{1}]} \right)
\end{align}
$$
资产回收量为
$$
\begin{equation}
Bo_{BI}[k_{deb},i_{ilq},t_{2}] = A_{BI}[i_{ilq},k_{deb},t_{1}] \; \dfrac{\Delta A_{BI}[i_{ilq},k_{deb},t_{2}]}{A_{B}[i_{ilq},t_{1}]}
\end{equation}
$$
...
$$
\begin{equation}
Shock_{B}[k_{deb},\tau+1] = Shock_{BI,run}[k_{deb},:,t_{2}] = \sum_{i \in \mathbb{B}_{ilq} \cap \mathbb{B}_{cre}}{Bo_{BI}[k_{deb},i,t_{2}]}
...
$$
\begin{equation}
Shock_{B}[k_{deb},\tau+1] = Shock_{BI,run}[k_{deb},:,t_{2}] = \sum_{i \in \mathbb{B}_{br} \cap \mathbb{B}_{cre}}{Bo_{BI}[k_{deb},i,t_{2}]}
...
Shock_{B,A_{BI}}[i,t_{1}][i,:,t_{2}] = \Delta A_{BI}[i,t_{2}] & i \in \mathbb{B}_{cre} \\
Shock_{B,Z_{BI}}[i,:,t_{2}] = \sum{Bo_{BI}[i,j,t_{2}]} & i \in \mathbb{B}_{deb} \cap \mathbb{B}_{hel} ,j \in (\mathbb{B}_{ilq} \cup \mathbb{B}_{br}) \cap \mathbb{B}_{cre} \\
\end{cases}
...
- [ ] 综合式$\eqref{破产银行资产清零}$$\eqref{资不抵债银行负债价值于资不抵债负债冲击之后}$$\eqref{债权银行之银行间资产变动}$，经过轮次$\tau_{i}$之后，银行间资产负债矩阵$\eqref{银行间资产负债网络}$之时变形式为：
...
\end{equation}
$$
根据式$\eqref{所有者权益时变之于银行间贷款损失冲击}$可得面临银行间贷款损失冲击造成银行$i$之银行内部分损失为
$$
\begin{equation}
