...
def content_InterBankIlliquidRepay(BB: BankCommercial, IB: BankInterbank, b: StateType, ib: StateType, para: dict, env: dict):
    ...
    Finance.update_finance_variables(BB, IB, b, ib, by_way='Shock_IB_run_ilq')
    BB.Shock_IB_run_ilq_s[b] -= BB.Li_IB_all[b]  # 各银行之银行间挤兑流动冲击源头变动，当收回相应的银行间贷款时
    Finance.update_finance_variables(BB, IB, b, ib, by_way='Shock_IB_run_ilq_s')
    ...
    Finance.update_finance_variables(BB, IB, b, ib, by_way='clear transfer all')  # 清零所有不必要的借贷流量变量；#BUG这个是否有必要？
