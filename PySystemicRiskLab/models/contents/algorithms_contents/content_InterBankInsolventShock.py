...
def content_InterBankInsolventShock(BB: BankCommercial, IB: BankInterbank, b: StateType, ib: StateType, para: dict, env: dict):
    ...
    Finance.update_variables(BB, IB, b, ib, by_way='A_IB_all')
    ...
    # BankState.update_B_state(BB, IB, target='insolvent', source='healthy')  # 更新各银行之状态，从健康到资不抵债 #FIXME
    ...
    Finance.update_variables(BB, IB, b, ib, by_way='Shock_IB_def_s')  # 更新违约损失冲击源头变量Shock_def_s
    ...
    Finance.update_variables(BB, IB, b, ib, by_way='Z_IB_all') # 更新资产负债表，通过Z_D或Z_IB_all
    ...
    Finance.update_variables(BB, IB, b, ib, by_way='Z_D') # 更新资产负债表，通过Z_D或Z_IB_all
