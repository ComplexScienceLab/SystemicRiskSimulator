...
class BankCommercial(BaseAgents):  # TODO有必要改成动态创建类属性
    ...
    A_all = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总资产 A_all: $A_all=A_IB+A_exIB$
    A_IB_all = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间资产加总 A_IB_all
    A_exIB = np.NaN  # = np.zeros((env['num_bank'], 1))  # 非银行间资产 A_exIB: $A_exIB=A_P+A_Q+A_R+A_other$
    ...
    Z_all = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总负债 Z_all: $Z_total=Z_IB+Z_exIB$
    Z_IB_all = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间负债加总 Z_IB_all
    Z_exIB = np.NaN  # = np.zeros((env['num_bank'], 1))  # 非银行间负债 Z_exIB: $Z_exIB=Z_D+Z_other$
    ...
    Shock_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总冲击目标 Shock_t $Shock_t = Shock_exIB_t+Shock_IB_t$
    Shock_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总冲击源头 Shock_s $Shock_s = Shock_exIB_s+Shock_IB_s$
    Shock_def_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总违约损失冲击目标 Shock_def_t $Shock_def_t = Shock_P_def_t+Shock_D_run_t$
    Shock_def_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总违约损失冲击源头 Shock_def_s $Shock_def_s = Shock_exIB_def_s+Shock_IB_def_s$
    Shock_run_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总挤兑流动冲击目标 Shock_run_t $Shock_run_t = Shock_exIB_run_t+Shock_IB_run_t$
    Shock_run_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 总挤兑流动冲击源头 Shock_run_s $Shock_run_s = Shock_exIB_run_s+Shock_IB_run_s$
    Shock_exIB_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 非银行间借贷冲击目标 Shock_exIB_t $Shock_exIB_t = Shock_P_def_t+Shock_D_run_t$
    Shock_exIB_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 非银行间借贷冲击源头 Shock_exIB_s $Shock_exIB_s = Shock_P_run_s+Shock_D_def_s$
    ...
    Shock_IB_def_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间违约损失冲击目标 Shock_IB_def_t
    Shock_IB_run_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间挤兑流动冲击源头 Shock_IB_run_s $Shock_IB_run_s=Shock_IB_run_ilq_s+Shock_IB_run_br_s$
    Shock_IB_run_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间挤兑流动冲击目标 Shock_IB_run_t $Shock_IB_run_t+Shock_IB_run_ilq_t+Shock_IB_run_br_t$
    Shock_IB_run_ilq_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间流动性短缺挤兑流动冲击源头 Shock_IB_run_ilq_s
    Shock_IB_run_ilq_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间流动性短缺挤兑流动冲击目标 Shock_IB_run_ilq_t
    Shock_IB_run_br_s = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间倒闭挤兑流动冲击源头 Shock_IB_run_br_s
    Shock_IB_run_br_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间倒闭挤兑流动冲击目标 Shock_IB_run_br_t
    Loss_IB = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间市场冲击损失 Loss_IB
    Loss_IB_def_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间资产负债违约冲击损失 Loss_IB_def_t
    Loss_IB_run_t = np.NaN  # = np.zeros((env['num_bank'], 1))  # 银行间负债流动性挤兑冲击损失 Loss_IB_run_t
    on = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否存在 is_on
    off = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否已退出不存在 is_off
    hel = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否健康 is_healthy
    isv = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否资不抵债 is_insolvent
    ilq = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否流动性短缺 is_illiquid
    br = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否破产 is_bankrupt
    is_needed_BoIB = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否需要偿还银行间借款 is_needed_BoIB
    is_enabled_BoIB = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否可以偿还银行间借款 is_enabled_BoIB
    is_needed_BoD = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否需要偿还居民部门存款 is_needed_BoD
    is_enabled_BoD = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否可以偿还居民部门存款 is_enabled_BoD
    is_needed_LiP = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否需要收回厂商贷款 is_needed_LiP
    is_enabled_LiP = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否可以收回厂商贷款 is_enabled_LiP
    is_allocated_Shock = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否已经分配传染冲击 is_allocated_Shock
...
class BankInterbank(BaseInterAgents):
    ...
    银行间邻接矩阵复合类
    """
    id_agent: IdsType = np.arange(1, env['num_bank'] * env['num_bank'] + 1).reshape((env['num_bank'], env['num_bank']))  # agent 之间之关联编号 id
...
if __name__ == "__main__":
    ...
    bankCommercial = BankCommercial(
        ...
        is_allocated_Shock=np.full(env['num_bank'], False),  # 示性向量之于银行是否已经分配传染冲击 is_allocated_Shock
    ...
    bankInterbank = BankInterbank(
        ...
        Loss_IB=np.zeros((env['num_bank'], env['num_bank'])),  # 银行间市场冲击损失 Loss_IB
        Loss_IB_def=np.zeros((env['num_bank'], env['num_bank'])),  # 银行间资产负债违约冲击损失 Loss_IB_def
        Loss_IB_run=np.zeros((env['num_bank'], env['num_bank'])),  # 银行间负债流动性挤兑冲击损失 Loss_IB_run
        is_exposure=np.zeros((env['num_bank'], env['num_bank'])),  # 信息邻接矩阵之于是否有银行间敞口 is_exposure
        on=np.full((env['num_bank'], env['num_bank']), True),  # 信息邻接矩阵之于银行间存在的 is_on
        off=np.full((env['num_bank'], env['num_bank']), False),  # 信息邻接矩阵之于银行间已退出不存在的 is_off
        hel=np.full((env['num_bank'], env['num_bank']), True),  # 信息邻接矩阵之于银行间健康的 is_healthy
        isv=np.full((env['num_bank'], env['num_bank']), False),  # 信息邻接矩阵之于银行间资不抵债的 is_insolvent
        ilq=np.full((env['num_bank'], env['num_bank']), False),  # 信息邻接矩阵之于银行间流动性短缺的 is_illiquid
        br=np.full((env['num_bank'], env['num_bank']), False),  # 信息邻接矩阵之于银行间破产的 is_bankrupt
