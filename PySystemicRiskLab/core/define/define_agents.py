...
#     is_needed_BoBI = np.NaN  # = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否需要偿还银行间借款 is_needed_BoBI
#     is_enabled_BoBI = np.NaN  # = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否可以偿还银行间借款 is_enabled_BoBI
...
class SystemicRiskAgent:
    ...
    id_agent: int  # 编号（必备的）
    BB: pd.Series  # 商业银行众
    BI: pd.Series  # 银行间邻接矩阵

    def __init__(self, id_agent, BB: pd.Series, BI: pd.Series):
