...
class BankCommercial(BaseAgents):
    ...
    Shock_def_t = np.zeros((env['num_bank'], 1))  # 总违约损失冲击目标 Shock_def_t $Shock_def_t = Shock_P_def_t+Shock_D_run_t$
    ...
    nBoBI = np.full((env['num_bank'], 1), False)  # 示性向量之于银行是否需要偿还银行间借款 isNeededBoBI
    eBoBI = np.full((env['num_bank'], 1), True)  # 示性向量之于银行是否可以偿还银行间借款 isEnabledBoBI
    ...
    listOfExist = np.full((env['num_bank'], 1), list)  # 列表之于存在的银行编号 listOfExist
    listOfInsolvent = np.full((env['num_bank'], 1), list)  # 列表之于资不抵债的银行编号 listOfInsolvent
    listOfIlliquity = np.full((env['num_bank'], 1), list)  # 列表之于流动性短缺的银行编号 listOfIlliquity
    listOfBankrupt = np.full((env['num_bank'], 1), list)  # 列表之于破产的银行编号 listOfBankrupt
