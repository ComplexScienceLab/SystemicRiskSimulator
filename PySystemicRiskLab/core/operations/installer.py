...
安装机
"""
from PySystemicRiskLab import deepcopy, Node, np, re, pd
...
class DataInstaller:
    ...
    def set_manually_values_to_Bank_variables(cls):
        """手动设置agent之变量于文件`set_agents_variables.py`，以初始化银行变量。"""
    ...
    def install_data(cls, init_method: str):
        ...
        # A = pd.Series([BB, BI], index=['BB', 'BI'])

        # # 初始化带回合变量的商业银行实例数组、初始化带回合变量的银行间市场实例数组 #HACK无用
        # A_data = Collector.collect(A, None, env)

        # ## 更新各银行之变量，在第一回合初始时
        # b = (BB.on | BB.off).values.reshape(-1, 1)  # 临时设置BB示性变量
        # ib = ((BB.on | BB.off).values.reshape(-1, 1) & (BB.on | BB.off).values.reshape(1, -1))  # 临时设置BI示性变量
