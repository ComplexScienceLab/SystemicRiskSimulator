...
# from PySystemicRiskLab.core import np,env,BankCommercial,BankInterbank,TypeState,LESS1,LESS2
import numpy as np
...
class BankState:
    ...
    @classmethod
    def init_listOfRelationInStateOfBanks(cls, bank: BankCommercial, interbank: BankInterbank):
        """
        初始化银行状态关系列表
        """
        interbank.cre = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.on, goal="creditor")
        interbank.deb = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.on, goal="debtor")
        pass
    ...
    @classmethod
    def calc_isHealthy_from_isIlliquity(cls, bank: BankCommercial, interbank: BankInterbank):
    ...
    @classmethod
    def update_isHealthy_from_isIlliquity(cls, bank: BankCommercial, interbank: BankInterbank):
        condition = (~(bank.isv | bank.ilq) & bank.on)
        if (bank.hel != condition).any():
            bank.hel = condition
            interbank.hel = (bank.hel & bank.hel.T)
    ...
    def calc_isInsolvent(cls, bank: BankCommercial, interbank: BankInterbank):
        ...
        if (bank.isv != condition).any():
            ...
            interbank.cre_isv = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.isv, goal="creditor")
            interbank.deb_isv = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.isv, goal="debtor")
            pass
        pass

    @classmethod
    def calc_isInsolvent_from_isHealthy(cls, bank: BankCommercial, interbank: BankInterbank):
        """算示性向量之于银行资不抵债的，来自健康的。"""
        condition = (((bank.A_all < bank.Z_all + LESS1) | (bank.E_all < LESS1) | (bank.Shock_def_t + LESS1 > bank.E_all)) & bank.on)
        if (bank.isv != condition).any():
            bank.isv = condition
            interbank.isv = (bank.isv | bank.isv.T)
    ...
    @classmethod
    def calc_isIlliquity(cls, bank: BankCommercial, interbank: BankInterbank):
        """算示性向量之于银行流动性短缺的。"""
        condition = ((bank.Shock_run_t + LESS1 >= bank.A_Q) & bank.on)
        if (bank.ilq != condition).any():
            bank.ilq = condition
            interbank.ilq = (bank.ilq | bank.ilq.T)
            interbank.cre_ilq = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.ilq, goal="creditor")
            interbank.deb_ilq = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.ilq, goal="debtor")
            pass
        pass
    ...
    @classmethod
    def calc_isIlliquity_from_isHealthy(cls, bank: BankCommercial, interbank: BankInterbank):
        condition = ((bank.Shock_run_t + LESS1 >= bank.A_Q) & bank.on)
        if (bank.ilq != condition).any():
            bank.ilq = condition
            interbank.ilq = (bank.ilq | bank.ilq.T)
    ...
    @classmethod
    def update_isIlliquity_from_isHealthy(cls, bank: BankCommercial, interbank: BankInterbank):
        condition = ((bank.Shock_run_t + LESS1 >= bank.A_Q) & bank.on)
        if (bank.ilq != condition).any():
            bank.ilq = condition
            interbank.ilq = (bank.ilq | bank.ilq.T)
    ...
    def calc_isBankrupt(cls, bank: BankCommercial, interbank: BankInterbank):
        ...
        if (bank.br != condition).any():
            ...
            interbank.cre_br = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.br, goal="creditor")
            interbank.deb_br = cls.calc_listOfRelationInStateOfBanks(interbank, isState=bank.br, goal="debtor")
            pass
        pass
    ...
    @classmethod
    def calc_isBankrupt_from_isIlliquity(cls, bank: BankCommercial, interbank: BankInterbank):
        condition = (bank.isv | bank.ilq)
        if (bank.br != condition).any():
            bank.br = condition
            interbank.br = (bank.br & bank.br.T)
    ...
    def together_isOn(cls, bank: BankCommercial, interbank: BankInterbank):
        ...
        if (bank.on != condition).any():
            ...
            # interbank.listOfCreditorsInOn = cls.calc_listOfRelationInStateOfBanks(interbank, isState = bank.on, goal = "creditor") #HACK，未定义，无用。
            # interbank.listOfDebtorsInOn = cls.calc_listOfRelationInStateOfBanks(interbank, isState = bank.on, goal = "debtor") #HACK，未定义，无用。
            pass
        pass


    @classmethod
    def update_isBankrupt_from_isOff(cls, bank: BankCommercial, interbank: BankInterbank):
    ...
    def calc_isEnabledBoD_from_isNeededBoD(cls, bank: BankCommercial, interbank: BankInterbank):
        """算示性向量之于银行能够偿还居民部门存款的，从需要偿还居民部门存款的。"""
    ...
    def calc_isEnabledLiP_from_isNeededLiP(cls, bank: BankCommercial, interbank: BankInterbank):
        """算示性向量之于银行能够收回厂商贷款的，从需要收回厂商贷款的。"""
        bank.eLiP = (bank.nLiP)  # HACK后续可能会补充条件 & producer.A_Q > 0
        pass


    @classmethod
    def calc_listOfRelationInStateOfBanks(cls, interbank: BankInterbank, isState: TypeState, goal: str):
        """
        计算信息列表之于各状态银行之各关联银行。
        ...
        Returns: listOfRelationInStateOfBanks
    ...
    def update_B_state(cls, bank: BankCommercial, interbank: BankInterbank, target: str = "any", source: str = "any"):
        ...
        if target == "any":  # FIXME 这个可能有缺陷:
            if source == "any":
                cls.init_listOfRelationInStateOfBanks(bank, interbank)
                cls.calc_isInsolvent(bank, interbank)
                cls.calc_isIlliquity(bank, interbank)
                ...
                cls.calc_isIlliquity_from_isHealthy(bank, interbank)
                cls.update_isHealthy_from_isIlliquity(bank, interbank)
                ...
                cls.calc_isHealthy_from_isIlliquity(bank, interbank)
                cls.update_isIlliquity_from_isHealthy(bank, interbank)
                cls.calc_isBankrupt_from_isIlliquity(bank, interbank)
