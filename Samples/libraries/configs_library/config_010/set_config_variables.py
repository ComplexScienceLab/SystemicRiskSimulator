...
set_config_variables = dict(
    ...
    simulator_version="v0.0.20_alpha",  # 模拟器版本号
    ...
    ## 使用的程序版本类型设置：
    is_use_flow_form_version_model=False,  # 是否使用流程形式的版本的模型形式。默认True。如果 False ，则表示不采用带有多文件多模块的旧版本的模型表示形式，而是直接采用简化的单文件模型。
    # is_use_flow_form_version_model=True,  # 是否使用流程形式的版本的模型形式。默认True。如果 False ，则表示不采用带有多文件多模块的旧版本的模型表示形式，而是直接采用简化的单文件模型。
