...
set_config_variables = dict(
    ...
    schedule_operation=dict(
        ...
        预处理实验结果程序=False,  # 默认 True
        # 可视化结果程序=True,  # 默认 False
        可视化结果程序=False,  # 默认 False
        # 分析实验结果程序=True,  # 默认 False
        分析实验结果程序=False,  # 默认 False
    ),

    list_agents_data_filename_para_01=['BB', 'IB', 'AB', 'note'],  # 设置 agents 初始数据列表
    ...
    folderpath_root_experiments=r"data/sims_debug",  # 手动设置实验文件夹根路径。默认"data/sims/"；
    foldername_experiments_output_data=r"exp_output_data",  # 手动设置实验导出数据文件夹名称。默认"exp_output_data"；
    folderpath_models=r"libraries/models_library/model_IB2111",  # 模型所在的文件夹
    folderpath_config=r"libraries/configs_library/config_sample_IB2111",  # 配置项设置所在的文件夹
    folderpath_parameters=r"libraries/parameters_library/parameters_sample_IB2111",  # 参数设置所在的文件夹
    folderpath_agents=r"libraries/agents_library/agents_sample_IB2111",  # 个体众数据初始化所在的文件夹
    ...
    list_idsExperiment_to_run=[95],  # 设置要运行的实验编号列表。默认 None，表示运行所有实验。
    # list_idsExperiment_to_run=[107],  # 设置要运行的实验编号列表。默认 None，表示运行所有实验。
    # list_idsExperiment_to_run=[46, 47, 48, 49, 50, 51, 56, 61, 67, 68],  # 设置要运行的实验编号列表。默认 None，表示运行所有实验。#DEBUG 用于调试
    # list_idsExperiment_to_run=list(range(1, 68 + 1)),  # 设置要运行的实验编号列表。默认 None，表示运行所有实验。#DEBUG 用于调试
    ...
    # NOTE 使用的模型类型类型设置：
    # is_use_Gym_environments=True,  # 是否使用 PettingZoo 环境框架。默认 True。
    is_use_Gym_environments=False,  # 是否使用 PettingZoo 环境框架。默认 True。
    # is_use_PettingZoo_environments=False,  # 是否使用 PettingZoo 环境框架。默认 True。 #HACK 这个用不到
    # is_use_RL_method=True,  # 是否使用强化学习方法。默认 True。
    is_use_RL_method=False,  # 是否使用强化学习方法。默认 True。
    # is_use_RLlib_framework=False,  # 是否使用 RLlib 强化学习框架。默认 False。 #HACK 这个用不到
    # RL_state='training',  # 强化学习状态：可选值包括 'training', 'using'。默认值 'using'； #HACK 注意，当只有处于 'using' 值的时候才会收集运行过程之数据。
    ...
    is_enable_multiprocessing_for_visualization=False,  # 可视化期间，是否启用多进程。默认 True；
    ...
    visulization_process=dict(
        ...
        # 绘制资产负债表图=False,  # 默认 True
        拼接资产负债表图=True,  # 默认 True
        # 拼接资产负债表图=False,  # 默认 True
        绘制矩阵热图=True,  # 默认 True
        # 绘制矩阵热图=False,  # 默认 True
        拼接矩阵热图=True,  # 默认 True
        # 拼接矩阵热图=False,  # 默认 True
    ...
    vis=dict(
        ...
        list_idsExperiment_to_vis=[95],  # 设置要可视化的实验编号列表。默认 None，表示可视化所有实验组。
