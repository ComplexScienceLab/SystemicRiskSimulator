...
def main():
    ...
    list_agents_networkDensity_IB = [0.25, 0.50, 0.75, 1.0]  # 设置 agents 初始数据列表
    # list_agents_networkDensity_IB = [0.5, 1.0]  # 设置 agents 初始数据列表
    ...
    for year in list_年份:
        for density_IB in list_agents_networkDensity_IB:
            ...
            # %% 设置 agent-based 相关变量
            num_banks_effected_by_banks = [np.sum(set_bankInterbank_variables['A_IB'][i] > 0.0) for i in range(num_bank)]
            num_banks_to_effect_banks = [np.sum(set_bankInterbank_variables['Z_IB'][i] > 0.0) for i in range(num_bank)]
            set_AB_variables = dict(
                id_agent=np.arange(num_bank),  # agent 之编号 id
                observations=[
                    {
                        'Loss_IB_def': [np.zeros(num_banks_effected_by_banks[i])],
                        'mask': [True],
                    }
                    for i in range(num_bank)
                ],  # 各银行之观察值 observations
                # observations=[],  # 各银行之观察值 observations
                # actions=[],  # 各银行之动作 actions
                actions=[
                    {
                        'theta_IB_def': [np.zeros(num_banks_to_effect_banks[i])],
                        'mask': [False],
                    }
                    for i in range(num_bank)
                ],  # 各银行之动作 actions
                rewards=[
                    ...
                    for i in range(num_bank)
                ],  # 各银行之奖励 rewards 值（类似效用函数）
                # rewards=[],  # 各银行之奖励 rewards 值（类似效用函数）
                dones=[
                    {
                        'values': [False],
                        'mask': [True],
            ...
            set_note_variables = dict(
                ...
                strategy_method=np.array(["preset", "preset", "preset", "preset", "preset"]),  # 各银行之智库之决策方式。可选值 'preset'、'learning'。默认值 'preset'
                strategy_intelligence=np.array(["normal", "normal", "normal", "normal", "normal"]),  # 各银行之智库之策略智能度。可选值 'excellence'、'normal'。默认值 'normal' 。当`strategy_method == 'preset'` 的时候，只能使用 'normal'。
                # strategy_Default_IB_def_s=np.array(["Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效
                # strategy_method=np.array(["preset", "preset", "preset", "preset", "preset"]),  # 各银行之智库之决策方式。可选值 'preset'、'learning'。默认值 'preset'
                strategy_Default_IB_def_s=np.array(["Strategy_均匀分布随机", "Strategy_应还尽还", "Strategy_均匀分布随机", "Strategy_应还尽还", "Strategy_均匀分布随机"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效 #DEBUG
                # strategy_Default_IB_def_s=np.array(["Strategy_等级完全相同", "Strategy_导入预置的", "Strategy_等级完全相同", "Strategy_导入预置的", "Strategy_等级完全相同"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效 #DEBUG
                # strategy_Default_IB_def_s=np.array(["Strategy_等级完全相同", "Strategy_等级完全相同", "Strategy_等级完全相同", "Strategy_等级完全相同", "Strategy_等级完全相同"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效 #DEBUG
                # strategy_Default_IB_def_s=np.array(["Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还", "Strategy_应还尽还"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效 #DEBUG
                # strategy_Default_IB_def_s=np.array(["Strategy_均匀分布随机", "Strategy_均匀分布随机", "Strategy_均匀分布随机", "Strategy_均匀分布随机", "Strategy_均匀分布随机"]),  # 各银行之智库之银行间在违约损失冲击的违约策略。当 `strategy_method == 'learning'` 的时候生效 #DEBUG
            ...
            with open(Path(folderpath_agents / f"id={id_agents}-v=BB-year={year}-density_IB={density_IB:.2f}.pkl"), 'wb') as f:
                pickle.dump(set_bankCommercial_variables, f)
            with open(Path(folderpath_agents / f"id={id_agents}-v=IB-year={year}-density_IB={density_IB:.2f}.pkl"), 'wb') as f:
                pickle.dump(set_bankInterbank_variables, f)
            with open(Path(folderpath_agents / f"id={id_agents}-v=AB-year={year}-density_IB={density_IB:.2f}.pkl"), 'wb') as f:
                pickle.dump(set_AB_variables, f)
            with open(Path(folderpath_agents / f"id={id_agents}-v=note-year={year}-density_IB={density_IB:.2f}.pkl"), 'wb') as f:
