...
if __name__ == '__main__':
    ...
    config['foldername_agents'] = "agents_sample_IB1111"
    ...
    for idx_item in range(num_row_agents):
        ...
        if '04' in sgv['外生违约损失冲击权重百分比方案']:
            ...
            # 设置组合的数量和大小
            num_combinations_for_scheme_04 = 50
        ...
        if proceed.lower() not in ['', 'yes']:
            ...
            if sgv['is_save_to_sqlite']:
                ...
                for index, row in df_parameters.iterrows():
                    ...
                    cur.execute(
                        '''
                        INSERT INTO parameters (exp_id, year, inner_id, Shock_exIB_def_t_percentage, description)
                        VALUES (?, ?, ?, ?, ?)
                        ''', (row['exp_id'], row['year'], row['inner_id'], Shock_exIB_def_t_percentage_str, description_str)
                    )
