...
if __name__ == '__main__':
    ...
    config['foldername_agents'] = "agents_sample_IB1211"
    ...
    # 银行抛售银行持有共同贷款资产之价格折扣率方案列表
    config['银行抛售银行持有共同贷款资产之价格折扣率方案'] = [
        # '01',
        '02',
    ]
    ...
    # 导入 agents 之参数
    df_agents_BB = pd.DataFrame(columns=['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA', 'agentsData'])
    df_agents_DA = pd.DataFrame(columns=['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA', 'agentsData'])
    df_agents_IB = pd.DataFrame(columns=['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA', 'agentsData'])
    df_agents_IBA = pd.DataFrame(columns=['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA', 'agentsData'])
    df_agents_note = pd.DataFrame(columns=['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA', 'agentsData'])
    ...
    for filename in filenames_agents:
        match = re.search(r'id=(\d*?)-v=(.*?)-year=(\d{4})-density_IB=(\d+\.\d+)-density_IBA=(\d+\.\d+)', str(filename))
        if match:
            id_agents = int(match.group(1))
            v = match.group(2)
            yearName = match.group(3)
            networkDensity_IB = float(match.group(4))
            networkDensity_IBA = float(match.group(5))
            list_agents_para.append((id_agents, v, yearName, networkDensity_IB, networkDensity_IBA))

    # 批量读取不同年份和网络密度下的 agents 数据文件（pkl 格式），并将其内容加载到对应的 DataFrame 中，以便后续参数组合和实验设计使用
    for (id_agents, v, yearName, networkDensity_IB, networkDensity_IBA) in list_agents_para:
        with open(Path(sgv['folderpath_agents'], 'agents', f"id={id_agents}-v={v}-year={yearName}-density_IB={networkDensity_IB:.2f}-density_IBA={networkDensity_IBA:.2f}.pkl"), 'rb') as f:
            agentsData = pickle.load(f)
            new_row = pd.DataFrame({'id_agents': id_agents, 'yearName': yearName, 'networkDensity_IB': networkDensity_IB, 'networkDensity_IBA': networkDensity_IBA, 'agentsData': [agentsData]})
    ...
    df_agents_DA = df_agents_DA.sort_values(by=['id_agents']).reset_index(drop=True)
    df_agents_IB = df_agents_IB.sort_values(by=['id_agents']).reset_index(drop=True)
    df_agents_IBA = df_agents_IBA.sort_values(by=['id_agents']).reset_index(drop=True)
    ...
    num_asset = dict()  # 资产个数
    num_所需资产 = dict()  # 所需资产个数
    idxs_所需资产 = dict()  # 所需资产索引位置
    for yearName in list_yearName:
        ...
        num_asset[yearName] = df_agents_note[(df_agents_note['yearName'] == yearName)]['agentsData'].values[0]['id_asset'].shape[0]
        num_所需资产[yearName] = 3
        idxs_所需资产[yearName] = np.array([0,1,2]) 
        pass  # for
    ...
    for idx_item in range(num_row_agents):
        ...
        if '01' in sgv['外生违约损失冲击权重百分比方案']:
            ...
            list_combinations_Shock_exIB_def_t_percentage_01 = np.array(np.meshgrid(*[Shock_exIB_def_t_percentage] * num_所需资产[yearName])).T.reshape(-1, num_所需资产[yearName])  # 生成所有可能的组合
        ...
        if '02' in sgv['外生违约损失冲击权重百分比方案']:
            ...
            for value in Shock_exIB_def_t_percentage:
                if value != 0:  # 我们已经添加了所有元素都为零的组合
                    for position in range(num_所需资产[yearName]):
                        combination = [0.0] * num_所需资产[yearName]
        ...
        if '03' in sgv['外生违约损失冲击权重百分比方案']:
            ...
            step = 0.05
            Shock_exIB_def_t_percentage = np.arange(0.0, 1.0 + step, step)
            list_combinations_Shock_exIB_def_t_percentage_03 = [(value,) * num_所需资产[yearName] for value in Shock_exIB_def_t_percentage]
        ...
        if '04' in sgv['外生违约损失冲击权重百分比方案']:
            ...
            combination_size = num_所需资产[yearName]
        ...
        # 将生成的组合插入到原来的所有资产索引中
        if num_asset[yearName] > num_所需资产[yearName]:
            list_combinations_Shock_exIB_def_t_percentage = insert_combinations_to_indices(num_asset[yearName], list_combinations_Shock_exIB_def_t_percentage, idxs_所需资产[yearName])
            pass  # if
    ...
    dict_list_description_kappa_BA_in_agents_items = dict()

    for idx_item in range(num_row_agents):
        id_agents = df_agents_BB.iloc[idx_item]['id_agents']
        yearName = df_agents_BB.iloc[idx_item]['yearName']
        networkDensity_IB = df_agents_BB.iloc[idx_item]['networkDensity_IB']
        networkDensity_IBA = df_agents_BB.iloc[idx_item]['networkDensity_IBA']
        agentsData_BB = df_agents_BB.iloc[idx_item]['agentsData']
        agentsData_IB = df_agents_IB.iloc[idx_item]['agentsData']

        # %%
        # #NOTE 方案 1：参数步进组合（该方案与其他方案互斥）
        if '01' in sgv['银行抛售银行持有共同贷款资产之价格折扣率方案']:
            logging.debug("方案 1：参数步进组合")
            step = 0.1
            kappa_BA = np.arange(0.0, 1.0 + step, step)
            # kappa_BA = np.arange(0.0, 1.0 + step, step)
            list_combinations_kappa_BA_01 = kappa_BA  # 生成所有可能的组合
            list_description_kappa_BA_01 = [f"银行抛售银行持有共同贷款资产之价格折扣率之各个个体步进组合"] * len(list_combinations_kappa_BA_01)  # 添加组合描述
            pass  # if

        # %%
        # #NOTE 方案 2：自定义组合
        if '02' in sgv['银行抛售银行持有共同贷款资产之价格折扣率方案']:
            logging.debug("方案 2：自定义组合")
            list_combinations_kappa_BA_02 = [
                0.0,
                0.5,
                1.0,
                2.0,
            ...
            #     1.0
            # ]
            list_description_kappa_BA_02 = [f"银行抛售银行持有共同贷款资产之价格折扣率之自定义组合"] * len(list_combinations_kappa_BA_02)  # 添加组合描述
            pass  # if

        # %%
        # 综合上述各个方案之组合
        list_combinations_kappa_BA = []
        list_description_kappa_BA = []
        for scheme in sgv['银行抛售银行持有共同贷款资产之价格折扣率方案']:
            list_combinations_kappa_BA.extend(eval(f'list_combinations_kappa_BA_{scheme}'))
            list_description_kappa_BA.extend(eval(f'list_description_kappa_BA_{scheme}'))
            pass  # if

        # 检查是否有重复的组合，如果有则去重
        array_combinations = np.array(list_combinations_kappa_BA)
        _, unique_indices = np.unique(array_combinations, axis=0, return_index=True)
        if len(unique_indices) < len(list_combinations_kappa_BA):
            logging.debug(f"年{yearName}密度{networkDensity_IB:.2f}有重复组合")
        list_combinations_kappa_BA = array_combinations[unique_indices].tolist()
        list_description_kappa_BA = [list_description_kappa_BA[i] for i in unique_indices]

        # 检查是否有全0的组合，如果有则去掉
        if any(np.count_nonzero(combination) == 0 for combination in list_combinations_kappa_BA):
            logging.debug(f"年{yearName}密度{networkDensity_IB:.2f}有全 0 组合")
            idx_all_zero_combinations = [idx for idx, combination in enumerate(list_combinations_kappa_BA) if np.count_nonzero(combination) == 0]  # 找到全 0 组合的索引
            list_combinations_kappa_BA = [x for i, x in enumerate(list_combinations_kappa_BA) if i not in idx_all_zero_combinations]  # 根据索引删除全 0 组合
            list_description_kappa_BA = [x for i, x in enumerate(list_description_kappa_BA) if i not in idx_all_zero_combinations]
            pass  # if

        dict_list_combinations_kappa_BA_in_agents_items[idx_item] = list_combinations_kappa_BA
        dict_list_description_kappa_BA_in_agents_items[idx_item] = list_description_kappa_BA
    ...
    try:
        ...
        if proceed.lower() not in ['', 'yes']:
            ...
            # %% #NOTE 普通版本的生成参数组合
            list_agents_params = df_agents_BB[['id_agents', 'yearName', 'networkDensity_IB', 'networkDensity_IBA']].values.tolist()
            ...
            for i, agentPara in enumerate(list_agents_params):
                ...
                if valid_combinations:
                    ...
                    df_parameters_for_each_agentsPara['density_IBA'] = [networkDensity_IBA] * len(combinations_for_each_agentsParam)
                    df_parameters_for_each_agentsPara['inner_id'] = list(range(len(combinations_for_each_agentsParam)))
                    df_parameters_for_each_agentsPara['Shock_exIB_def_t_percentage'] = [x[0] for x in combinations_for_each_agentsParam]
                    df_parameters_for_each_agentsPara['kappa_BA'] = [x[1] for x in combinations_for_each_agentsParam]
                    ...
                    df_parameters_for_each_agentsPara['description'] = [
                        f"个体集{id_agents}年份{yearName}银行间密度{networkDensity_IB}与银行与资产密度之{networkDensity_IBA}之：" + "，".join(str(xx) for xx in x) + "。" for x in combinations_for_each_description
                    ]
            ...
            if sgv['is_save_to_sqlite']:
                ...
                for index, row in df_parameters.iterrows():
                    ...
                    cur.execute(
                        '''
                        INSERT INTO parameters (exp_id, year, inner_id, Shock_exIB_def_t_percentage, kappa_BA, description)
                        VALUES (?, ?, ?, ?, ?, ?)
                        ''', (row['exp_id'], row['year'], row['inner_id'], Shock_exIB_def_t_percentage_str, row['kappa_BA'], description_str)
                    )
