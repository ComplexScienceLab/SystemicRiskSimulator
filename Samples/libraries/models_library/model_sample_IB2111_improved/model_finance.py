...
class ModelFinance:
    ...
    def __init__(self, num_bank: int):
        ...
        self.dict_update_methods = {  # 创建一个字典，映射 by_way 的值到对应的方法
            ## NOTE：功能函数集：更新冲击、状态。
    ...
    def together_B_A_all(self, A: ModelAgent):
        """汇总各银行之总资产。"""
        A.BB.A_all[A.BB.exist] = A.BB.A_IB_all[A.BB.exist] + A.BB.A_P[A.BB.exist] + A.BB.A_Q[A.BB.exist] + A.BB.A_R[A.BB.exist] + A.BB.A_I[A.BB.exist] + A.BB.A_other[A.BB.exist]
    ...
    def together_B_Z_all(self, A: ModelAgent):
        """汇总各银行之总负债。"""
        A.BB.Z_all[A.BB.exist] = A.BB.Z_IB_all[A.BB.exist] + A.BB.Z_D[A.BB.exist] + A.BB.Z_CB[A.BB.exist] + A.BB.Z_I[A.BB.exist] + A.BB.Z_other[A.BB.exist]
    ...
    def calc_B_E_all(self, A: ModelAgent):  # BUG是否考虑E_all负数？还是手动计算？
        ...
        A.BB.E_all[A.BB.exist] = A.BB.A_all[A.BB.exist] - A.BB.Z_all[A.BB.exist] - (np.zeros(self.num_bank) + 1e-8)[A.BB.exist]  # 允许E_all为负数
    ...
    def calc_state_healthy(self, A: ModelAgent):
        """计算示性向量之于银行健康的。"""
        result = (
                (A.BB.E_all >= np.zeros(self.num_bank) + 1e-8) &
                (A.BB.A_Q >= np.zeros(self.num_bank) + 1e-8) &
                # (A.BB.Shock_def_t + (np.zeros(self.num_bank) + 1e-8) <= A.BB.E_all) &
                # (A.BB.Default + (np.zeros(self.num_bank) + 1e-8) <= A.BB.E_all) &
                (A.BB.Loss_t + (np.zeros(self.num_bank) + 1e-8) <= A.BB.E_all) &
                (A.BB.exist)
        )
    ...
    def calc_state_insolvent(self, A: ModelAgent):
        ...
        result = (
                (
                        (A.BB.A_all < A.BB.Z_all + np.zeros(self.num_bank) + 1e-8) |
                        (A.BB.E_all < np.zeros(self.num_bank) + 1e-8) |
                        # (A.BB.Shock_def_t + (np.zeros(self.num_bank) + 1e-8) > A.BB.E_all) |
                        # (A.BB.Default + (np.zeros(self.num_bank) + 1e-8) > A.BB.E_all) |
                        (A.BB.Loss_t + (np.zeros(self.num_bank) + 1e-8) > A.BB.E_all)
    ...
    def update_variables(self, A, by_way: str = 'all'):
        ...
        # 使用字典来调用对应的方法
        if by_way in self.dict_update_methods:
            self.dict_update_methods[by_way](A)
        else:
