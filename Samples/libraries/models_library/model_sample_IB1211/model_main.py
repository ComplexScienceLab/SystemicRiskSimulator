...
class ModelMain:
    ...
    def __init__(
        ...
        self.A_data = A_data
        self.para = para
        self.model_finance = model_dict['model_finance'](sgv['num_bank'])  # 初始化 Content_Finance 之实例
    ...
    def model_content(self):
        ...
        if self.sgv['turn'] > 0:
            ...
            self.sgv['process_name'] = "HoldingCoLoanAssetPriceContagionShock"
            self.sgv['turn'] += 1  # 回合数计次轮次数（由于开始轮次是`START`，所以记为0）
            self.sgv['phase'] = 1  # 逐相复位（起始为1）
            logging.debug(f"          轮次 {self.sgv['turn']}：模型 {self.sgv['process_name']}")
            ...
            # self.A.DA.A = np.maximum(np.sum(self.A.IBA.IBA[:, self.A.DA.inf], axis=0), 0.0001)  # 自动计算各贷款类资产资产之总量
            Executer.update_variableStep(self.model_finance, 'Shock_IBA_def', self.A, self.A_data, self.para, self.sgv, is_collect=self.sgv['is_collect'], collect=self.sgv['collect'])  # 计算银行持有共同贷款资产违约损失冲击目标

            self.A.IBA.IDA_exp_factor = np.zeros((self.A.note.num_bank, self.A.note.num_asset))  # 银行持有贷款类资产单价变动后的值
            self.A.IBA.IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)] = np.exp(-self.A.DA.kappa_DA * (self.A.IBA.IBA[np.ix_(self.A.BB.inf, self.A.DA.inf)] / self.A.DA.DA[self.A.DA.inf]))  # 计算银行持有贷款类资产之指数传染因子

            # 计算银行受到持有共同贷款资产价格损失冲击
            self.A.BB.Shock_IBA_def_t = np.zeros(self.A.note.num_bank)
            self.A.IBA.Shock_IBA_def[:, self.A.DA.inf] = self.A.IBA.IBA[:, self.A.DA.inf] * (1 - self.A.IBA.IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)]).sum(axis=0)  # 计算银行持有贷款类资产之冲击
            Executer.update_variableStep(self.model_finance, 'Shock_IBA_def', self.A, self.A_data, self.para, self.sgv, is_collect=self.sgv['is_collect'], collect=self.sgv['collect'])  # 更新银行持有共同贷款资产违约损失冲击
            # self.A.BB.Shock_IBA_def_t = (self.A.IBA.IBA[:, self.A.DA.inf] @ (1 - self.A.IBA.IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)]).T).sum(axis=1)  # 计算银行持有共同贷款资产违约损失冲击目标
            # Executer.update_variableStep(self.model_finance, 'Shock_IBA_def_t', self.A, self.A_data, self.para, self.sgv, is_collect=self.sgv['is_collect'], collect=self.sgv['collect'])

            # 计算银行持有共同资产之损失量
            # self.A.IBA.Loss_BA += np.sum(self.A.IBA.IBA * self.A.IBA.Shock_A, axis=1)
            self.A.IBA.Loss_IBA += self.A.IBA.Shock_IBA_def
            self.A.BB.Loss_IBA_def_t += self.A.BB.Shock_IBA_def_t

            last_IBA = self.A.IBA.IBA.copy()
            # logging.debug(last_IBA)

            self.A.IBA.IBA[:, self.A.DA.inf] = self.A.IBA.IBA[:, self.A.DA.inf] * np.prod(self.A.IBA.IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)], axis=0)  # 计算银行持有贷款类资产
            # self.A.IBA.IBA[np.ix_(self.A.BB.inf, self.A.DA.inf)] = self.A.IBA.IBA[np.ix_(self.A.BB.inf, self.A.DA.inf)] * np.prod(self.A.IBA.IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)], axis=0)  # 计算银行持有贷款类资产  #BUG 这个是错的。
            # logging.debug(self.A.IBA.IBA)
            # logging.debug(f"last_IBA - self.A.IBA.IBA:\n {last_IBA - self.A.IBA.IBA}")
            #
            # logging.debug(f"self.A.BB.Shock_IBA_def_t:\n {self.A.BB.Shock_IBA_def_t}")

            Executer.update_variableStep(self.model_finance, 'IBA', self.A, self.A_data, self.para, self.sgv, is_collect=self.sgv['is_collect'], collect=self.sgv['collect'])  # 更新银行持有共同贷款资产
            # logging.debug(f"self.A.DA.DA:\n {self.A.DA.DA}")

            # # 自动更新银行受到的叠加冲击
            # self.A.BB.Shock_t = self.A.BB.Shock_IB_def_t + self.A.BB.Shock_IBA_def_t

            # 更新示性向量之于银行当前轮次是否传染出去
            self.A.BB.con = ~self.A.BB.br & (self.A.BB.Shock_t > 0.0)
            logging.debug(f"          轮次 {self.sgv['turn']}：模型 {self.sgv['process_name']}，银行传染出去情况：{self.A.BB.con}")
            ...
            self.A.DA.Shock_DA_def_t = self.A.DA.DA * np.array(self.para['Shock_exIB_def_t_percentage'])  # 贷款资产之外生违约损失冲击

            self.A.DA.inf = self.A.DA.Shock_DA_def_t > 0.0  # 示性向量之贷款资产当前轮次遭受传染

            self.A.DA.p[self.A.DA.inf] = (self.A.DA.DA[self.A.DA.inf] - self.A.DA.Shock_DA_def_t[self.A.DA.inf]) / self.A.DA.DA[self.A.DA.inf]  # 贷款资产单位价格变动

            self.A.IBA.Shock_IBA_def[:, self.A.DA.inf] = self.A.IBA.IBA[:, self.A.DA.inf] * (1 - self.A.DA.p[self.A.DA.inf])  # 计算银行持有共同贷款类资产之违约损失冲击
            Executer.update_variableStep(self.model_finance, 'Shock_IBA_def', self.A, self.A_data, self.para, self.sgv, is_collect=self.sgv['is_collect'], collect=self.sgv['collect'])
            ...
            self.A.BB.Shock_P_def_t[self.A.BB.exist] = self.A.BB.Shock_IBA_def_t[self.A.BB.exist]  # 银行持有贷款类资产之违约损失冲击
            ...
            # node_015 外部资产违约损失冲击
            IDA_exp_factor = np.zeros((self.A.note.num_bank, self.A.note.num_asset))  # 银行持有贷款类资产之指数传染因子
            self.A.DA.kappa_DA = self.para['kappa_BA']  #
            IDA_exp_factor[np.ix_(self.A.BB.inf, self.A.DA.inf)] = np.exp(-self.A.DA.kappa_DA * self.A.IBA.IBA[np.ix_(self.A.BB.inf, self.A.DA.inf)] / np.maximum(np.sum(self.A.IBA.IBA[:, self.A.DA.inf], axis=0), 0.0001))  # 计算银行持有贷款类资产之指数传染因子
