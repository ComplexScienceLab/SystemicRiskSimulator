...
class ModelFinance:
    ...
    def together_Shock_target(self, A: ModelAgent):
        """汇总冲击目标"""
        A.BB.Shock_t[A.BB.exist] = A.BB.Shock_P_def_t[A.BB.exist] + A.BB.Shock_IB_def_t[A.BB.exist] + A.BB.Shock_IBA_def_t[A.BB.exist]
        logging.debug(f"                    together_Shock_target")
        pass  # function

    def together_Shock_source(self, A: ModelAgent):
        """汇总冲击源头"""
        A.BB.Shock_s[A.BB.exist] = A.BB.Shock_D_def_s[A.BB.exist] + A.BB.Shock_IB_def_s[A.BB.exist] + A.BB.Shock_IBA_def_s[A.BB.exist]
    ...
    def sum_Shock_IBA_def_target(self, A: ModelAgent):
        """汇总银行持有共同贷款类资产违约损失冲击目标。"""
        A.BB.Shock_IBA_def_t[A.BB.exist] = np.sum(A.IBA.Shock_IBA_def * A.IBA.exist, axis=1)[A.BB.exist]
        pass
    ...
    ## NOTE 功能函数集：计算银行持有资产
    def sum_DA_IBA(self, A: ModelAgent):
        """加总各银行之持有资产。"""
        A.DA.DA = np.maximum(np.sum(A.IBA.IBA, axis=0), 0.0001)  # 不为0的原因是为了避免除数为0
        logging.debug(f"                    sum_DA_IBA")
        pass
    ...
    def update_by_Shock_IBA_def_t(self, A: ModelAgent):
        self.together_Shock_target(A)
        pass  # function

    def update_by_Shock_IBA_def(self, A: ModelAgent):
        self.sum_Shock_IBA_def_target(A)
        self.together_Shock_target(A)
        pass  # function
    ...
    ## NOTE：功能函数集：更新银行持有资产
    def update_by_IBA(self, A: ModelAgent):
        self.sum_DA_IBA(A)
        pass  # function
    ...
    def update_variables(self, A, by_way: str = 'all'):
        ...
        - 商业银行之冲击部分：
            ...
            - ``Shock_IBA_def_t``:  已知``Shock_{IBA,def}[j,: }],:  \\in i_{isv}``，更新其余冲击变量；
        ...
        dict_update_methods = {  # 创建一个字典，映射 by_way 的值到对应的方法
            ...
            'Shock_IBA_def_t': self.update_by_Shock_IBA_def_t,
            'Shock_IBA_def': self.update_by_Shock_IBA_def,
