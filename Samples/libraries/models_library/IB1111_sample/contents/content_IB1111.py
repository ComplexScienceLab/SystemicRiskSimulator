...
def content_IB1111(A: SystemicRiskAgent, A_data: AgentDataCollection, para: dict, sgv: dict):
    ...
    # Executer.step_update('clear Shock_IB and Shock_exIB', A, para, sgv)
...
def ExBankInsolventShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    # Executer.step_update('clear all Shock_source', A, para, sgv)  # 清零所有冲击源头变量 Shock_source
    # Executer.step_update('clear all Shock_IB', A, para, sgv)  # 清零所有银行间冲击变量 Shock_IB
    ...
    Executer.step_update('Loss_exIB_def_t', A, para, sgv)
...
def InterBankInsolventContagion(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.step_update('Loss_IB_def', A, para, sgv)
...
def ExBankIlliquidShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    for i in np.where(A.BB.ilq)[0]:  # 流动性短缺银行计划收回资产，导致其对各债务银行之资产变动，造成流动性短缺银行间挤兑流动冲击
        ...
        pass
    Executer.step_update('Shock_IB_run_ilq', A, para, sgv)  # 更新挤兑流动冲击源头变量Shock_run_s
...
def InterBankIlliquidAllocate(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.step_update('enabled collect A_P', A, para, sgv)  # 计算是否可以收回厂商贷款状态
...
def InterBankIlliquidRepay(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.step_update('A_Q', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？
    Executer.step_update('Shock_D_run_t', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？
