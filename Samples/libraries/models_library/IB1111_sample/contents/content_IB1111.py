...
def content_IB1111(A: SystemicRiskAgent, A_data: AgentDataCollection, para: dict, sgv: dict):
    ...
    # Executer.variable_step_update('clear Shock_IB and Shock_exIB', A, para, sgv)
...
def ExBankInsolventShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('Shock_IB_def_s', A, para, sgv)  # 更新违约损失冲击源头变量Shock_def_s
    ...
    Executer.variable_step_update('Shock_D_def_s', A, para, sgv)  # 更新违约损失冲击源头变量Shock_def_s
    A.BB.Z_IB_all[A.BB.isv] -= A.BB.Shock_IB_def_s[A.BB.isv]  # 银行间负债变动，由于违约
    Executer.variable_step_update('Z_IB_all', A, para, sgv)  # 更新资产负债表，通过Z_D或Z_IB_all
    A.BB.Z_D[A.BB.isv] -= A.BB.Shock_D_def_s[A.BB.isv]  # 存款负债变动，由于违约
    Executer.variable_step_update('Z_D', A, para, sgv)  # 更新资产负债表，通过Z_D或Z_IB_all
...
def InterBankInsolventContagion(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('Z_IB', A, para, sgv)
    Executer.variable_step_update('Shock_IB_def', A, para, sgv)
    Executer.variable_step_update('Loss_IB_def', A, para, sgv)
...
def InterBankInsolventShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('A_IB_all', A, para, sgv)  # 更新银行间资产
    A.BB.E_all[A.BB.on] = np.maximum(A.BB.E_all[A.BB.on] - A.BB.Shock_def_t[A.BB.on], 0.0)  # 银行之所有者权益变动
    A.BB.Shock_IB_def_s[A.BB.isv] = abs((A.BB.Shock_def_t[A.BB.isv] - A.BB.E_all[A.BB.isv]) / (A.BB.Z_IB_all[A.BB.isv] + A.BB.Z_D[A.BB.isv]) * A.BB.Z_IB_all[A.BB.isv])  # 计算应银行内冲击传导至银行间传染冲击
    Executer.variable_step_update('Shock_IB_def_s', A, para, sgv)  # 更新违约损失冲击源头变量Shock_def_s
    A.BB.Shock_D_def_s[A.BB.isv] = abs((A.BB.Shock_def_t[A.BB.isv] - A.BB.E_all[A.BB.isv]) / (A.BB.Z_IB_all[A.BB.isv] + A.BB.Z_D[A.BB.isv]) * A.BB.Z_D[A.BB.isv])  # 计算应银行内冲击传导至银行存款传染冲击
...
def ExBankIlliquidShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('Shock_D_run_t', A, para, sgv)  # 居民存款挤兑流动冲击传导至银行内资产冲击
    ...
    Executer.variable_step_update('Shock_IB_run_ilq_s', A, para, sgv)  # 更新挤兑流动冲击源头变量 Shock_run_t
    ...
    for i in np.where(A.BB.ilq)[0]:  # 流动性短缺银行计划收回资产，导致其对各债务银行之资产变动，造成流动性短缺银行间挤兑流动冲击
        ...
        pass
    Executer.variable_step_update('Shock_IB_run_ilq', A, para, sgv)  # 更新挤兑流动冲击源头变量Shock_run_s
...
def InterBankIlliquidContagionShock(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('Shock_IB_run_ilq_s', A, para, sgv)  # 更新挤兑流动冲击源头变量Shock_run_t
    ...
    for i in np.where(i_nas)[0]:  # 流动性短缺银行计划收回资产，导致其对各债务银行之资产变动，造成流动性短缺银行间挤兑流动冲击
        A.IB.Shock_IB_run_ilq[A.IB.deb_ilq[i], i] = A.IB.A_IB[i, A.IB.deb_ilq[i]] * A.BB.Shock_IB_run_ilq_s[i] / A.BB.A_IB_all[i]
        pass
    Executer.variable_step_update('Shock_IB_run_ilq', A, para, sgv)  # 更新挤兑流动冲击源头变量Shock_run_t
...
def InterBankIlliquidAllocate(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('Li_P', A, para, sgv)

    Executer.variable_step_update('enabled repay Z_D', A, para, sgv)  # 计算是否需要偿还借款状态
    Executer.variable_step_update('enabled repay IB', A, para, sgv)  # 计算是否可以偿还银行间借款状态
    ...
    Executer.variable_step_update('Bo_D', A, para, sgv)  # HACK 这个必须放在这里！
    Executer.variable_step_update('Bo_IB_all', A, para, sgv)
    for i in np.where(A.BB.is_enabled_BoIB)[0]:  # 计算银行可以偿还各债权银行的借款流量
        ...
        pass
    Executer.variable_step_update('Bo_IB', A, para, sgv)
...
def InterBankIlliquidRepay(A: SystemicRiskAgent, para: dict, sgv: dict):
    ...
    Executer.variable_step_update('A_P', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？
    Executer.variable_step_update('Shock_P_run_s', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？
    ...
    Executer.variable_step_update('Z_D', A, para, sgv)
    Executer.variable_step_update('A_Q', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？
    Executer.variable_step_update('Shock_D_run_t', A, para, sgv)  # DEBUG 是否已经在自动更新功能计算过了，以至于重复计算了？

    A.IB.Shock_IB_run_ilq[A.ib] -= A.IB.Bo_IB[A.ib]  # 各银行间挤兑流动冲击变动，当偿还相应的银行间借款时
    Executer.variable_step_update('Shock_IB_run_ilq', A, para, sgv)
    A.BB.Shock_IB_run_ilq_s[A.b] -= A.BB.Li_IB_all[A.b]  # 各银行之银行间挤兑流动冲击源头变动，当收回相应的银行间贷款时
    Executer.variable_step_update('Shock_IB_run_ilq_s', A, para, sgv)
    ...
    # Executer.variable_step_update('clear transfer all', A, para, sgv)  # 清零所有不必要的借贷流量变量；
