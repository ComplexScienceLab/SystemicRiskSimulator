...
df_combinationOfPara[!, "id"] = collect(range(1:length(df_combinationOfPara))) # 添加id
wsave(datadir(env[:folderpathOfExperimentsData], savename(list_combinationOfPara, ".csv")), list_combinationOfPara) # 导出字段列表为csv格式
