...
root_dir_of_experiments = projectdir() * "/data/sims"; # 手动设置实验文件夹根路径。默认projectdir( * "/data/sims/"；
...
state_of_schedule = :idle; # 调度运作状态。状态符有以下几种：`:idle`：闲置状态、`:indexing`：索引状态、`:stepping`：步进状态、`:saving`：存储状态、`:loading`：读取状态、`:collecting`：收集数据状态、`:running`：运行状态；
state_of_process = :initializing; # 程序所处时期状态。状态符有以下几种：`:initializing`：初始化状态、`:running`：运行状态、`:finishing`：结束收尾状态、
